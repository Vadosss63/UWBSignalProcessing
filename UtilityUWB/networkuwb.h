...
template <typename Data, typename Header> union DataSend {
  struct Message {
    // заголовок пакета
    Header header;
    // данные
...
public:
  ...
  template <typename T> bool SendData(const std::vector<T> &data) noexcept;
  ...
  template <typename SimpleStruct> SimpleStruct GetData() const;
...
bool ReciverUDP::SendAnswer(const SimpleStruct &data) noexcept {
  ClearErrors();
  m_socket->send_to(boost::asio::buffer(reinterpret_cast<uint8_t *>(&data),
                                        sizeof(SimpleStruct)),
                    m_epSender, 0, GetErrors());
  return IsNoError();
}

template <typename SimpleStruct> SimpleStruct ReciverUDP::GetData() const {
  SimpleStruct simpleStruct;
  memcpy(reinterpret_cast<uint8_t *>(&simpleStruct), m_bufferReciveData.Data(),
         m_sizeRecivedByte);
  return simpleStruct;
}
...
bool SenderUDP::SendData(const SimpleStruct &data) noexcept {
  ClearErrors();
  m_sock.send_to(boost::asio::buffer(reinterpret_cast<const uint8_t *>(&data),
                                     sizeof(SimpleStruct)),
                 m_ep, 0, GetErrors());
  return IsNoError();
}

template <typename SimpleStruct> SimpleStruct SenderUDP::GetData() const {
  SimpleStruct simpleStruct;
  memcpy(reinterpret_cast<uint8_t *>(&simpleStruct), m_bufferReciveData.Data(),
         m_sizeRecivedByte);
  return simpleStruct;
