...
{
    m_ep = tcp::resolver::endpoint_type(boost::asio::ip::address_v4::from_string(ip) , port);
}
...
void FileSender::HandleResolve(const boost::system::error_code& err, tcp::resolver::iterator endpoint_iterator)
{
    if(!err)
    {
        tcp::endpoint endpoint = *endpoint_iterator;
        m_socket.async_connect(endpoint,
                               boost::bind(&FileSender::HandleConnect,
                                           this,
...
void FileSender::SendRequestReciveFile()
{
    boost::asio::async_write(m_socket,
                             ...
                             boost::bind(&FileSender::HandleWriteFile,
                                         this,
                                         boost::asio::placeholders::error));
}

void FileSender::HandleConnect(const boost::system::error_code& err, tcp::resolver::iterator endpoint_iterator)
{
    if(!err)
    {
    ...
    else if(endpoint_iterator != tcp::resolver::iterator())
    {
        m_socket.close();
        tcp::endpoint endpoint = *endpoint_iterator;
        m_socket.async_connect(endpoint, boost::bind(&FileSender::HandleConnect, this, boost::asio::placeholders::error, ++endpoint_iterator));
    }
    else
    {
        std::cout << "Error: " << err.message() << std::endl;
...
void FileSender::HandleWriteFile(const boost::system::error_code& err)
{
    if(!err)
    {
        if(m_sourceFile)
        {
            ...
            std::cout << "Send " << m_sourceFile.gcount() << " bytes, total: " << m_sourceFile.tellg() << " bytes.\n";

            // отправляем данные полуателю
            boost::asio::async_write(m_socket,
                                     boost::asio::buffer(m_buf.c_array(), m_sourceFile.gcount()),
                                     boost::bind(&FileSender::HandleWriteFile, this, boost::asio::placeholders::error));
        }
        else
        {
            if(SetupFile())
...
{
    // читаем заголовок
    m_socket->async_receive(boost::asio::buffer(m_buf.c_array(),
                                                MAX_PATH_CHAR + sizeof (HeaderFile)),
...
void FileReciver::AsyncRead(size_t size)
{
    m_socket->async_receive(boost::asio::buffer(m_buf.c_array(), size),
                            [&] (const boost::system::error_code& error, size_t s)
    {
        HandleReadFileContent(error, s);
    });
}

void FileReciver::HandleReadRequest(const boost::system::error_code& err, std::size_t bytes_transferred)
{
    if(err)
...
void FileReciver::HandleReadFileContent(const boost::system::error_code& err, std::size_t bytes_transferred)
{
    ...
    {
        size_t sizeBuffer = (m_fileSize - sizeCurrent) > MAX_SIZE_BUF ? MAX_SIZE_BUF : (m_fileSize - sizeCurrent);
        AsyncRead(sizeBuffer);
    }

}

void FileReciver::HandleError(const std::string& function_name, const boost::system::error_code& err)
{
