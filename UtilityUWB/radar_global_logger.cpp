...
BOOST_LOG_GLOBAL_LOGGER_INIT(logger, src::severity_logger_mt) {
    ...
    auto logFileName = "logs/log_" + std::to_string(1900 + parts->tm_year) + "_" + std::to_string(1 + parts->tm_mon) + "_" + std::to_string(parts->tm_mday) + ".log";
    sink->locked_backend()->add_stream(boost::make_shared<std::ofstream>(logFileName, std::ios::out | std::ios::app));

    //    // add "console" output stream to our sink
    //    boost::shared_ptr<std::ostream> consoleStream(&std::clog, boost::null_deleter());
