...
{
    std::string data(reinterpret_cast<char*>(m_bufferReciveData.Data()), m_sizeRecivedByte);
    return data;
}
...
ReciverUDP::ReciverUDP(): m_service(new IOservice), m_socket(new Socket(*m_service)),
    m_isStart(false){}

ReciverUDP::ReciverUDP(uint16_t port):
    m_service(new IOservice), m_socket(new Socket(*m_service)), m_isStart(false)
{
    SetEndPoint("127.0.0.1", port);
}
...
ReciverUDP::ReciverUDP(const std::string& adress, uint16_t port, uint16_t sizeBuf):
    m_service(new IOservice), m_socket(new Socket(*m_service)), m_isStart(false), m_bufferReciveData(sizeBuf)
{
    SetEndPoint(adress, port);
}
...
void ReciverUDP::SetEndPoint(const std::string& adress, uint16_t port) noexcept
{
...
uint8_t* BufferReciveData::Data() const
{
    return m_bufferRecive;
}

size_t BufferReciveData::Size() const
{
