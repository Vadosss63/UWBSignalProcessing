...
void SchemeWindow::SaveScheme()
{
    ...
    QString pathName = m_switchCurrentSchema->tabText(m_switchCurrentSchema->currentIndex());

    QString path = QFileDialog::getSaveFileName(this, "Сохранение файла", pathName,
                                                "uwbs(*.uwbs)");
    path = path.trimmed();
...
void SchemeWindow::RemoveScheme()
{
    ...
    if(index < sizeSchemes)// для проверки, что это не кнопка добавления новой схемы
...
void SchemeWindow::RemoveSchemeScene(int index)
{
    ...
    int currInd = m_switchCurrentSchema->currentIndex(); //для установки соседней схемы, но не затрагивая кнопку добавления сигнала.
...
{
    QString path = QFileDialog::getOpenFileName(this, "Файл стиля","",
                                                "qss(*.qss)");
    if(!path.isEmpty())
...
void SchemeWindow::AddViewOfSchema(const QString& nameSchema)
{
    ...
    connect(schemaView->GetSchemaController(), SIGNAL(selectionChanged()),
            this, SLOT(UpdateActionsMenu()), Qt::QueuedConnection);

    connect(schemaView->GetSchemaController(), SIGNAL(StopOperation()),
            this, SLOT(StopShemaScript()), Qt::QueuedConnection);

    connect(schemaView->GetSchemaController(), SIGNAL(StartOperation()),
            this, SLOT(UpdateActionsMenu()), Qt::QueuedConnection);
...
void SchemeWindow::RenameTabDialog(int index)
{
    ...
    if((index  != i) && m_selectionPanelElementsButton->isEnabled())
    {
        ...
        renameSchemeDialog.setStyleSheet(styleSheet());
        renameSchemeDialog.SetTextLine(m_switchCurrentSchema->tabText(m_switchCurrentSchema->currentIndex()));
        if(renameSchemeDialog.exec())
...
{
    if(nameSchema.isEmpty()) return;
    SchemaView* schemaView = GetCurrentSchema();
    schemaView->RenameScheme(nameSchema);
    m_switchCurrentSchema->setTabText(m_switchCurrentSchema->currentIndex(), nameSchema);
...
void SchemeWindow::ReloudBlocks()
{
    m_itemsDialogWidget->ReloudBlocks();
}
...
{
    m_openFileSchemeButton = new QAction(QIcon(":/images/document-open.svg"), "Открыть", this);
    m_openFileSchemeButton->setShortcut(QKeySequence::Open);
    m_openFileSchemeButton->setStatusTip("Открыть схему");
    connect(m_openFileSchemeButton, SIGNAL(triggered()), this, SLOT(OpenScheme()));

    m_saveFileSchemaButton = new QAction(QIcon(":/images/document-save.svg"),"Сохранить", this);
    m_saveFileSchemaButton->setShortcut(QKeySequence::Save);
    m_saveFileSchemaButton->setStatusTip("Сохранить схему");
    connect(m_saveFileSchemaButton, SIGNAL(triggered()), this, SLOT(SaveScheme()));

    m_addSchemeButton = new QAction(QIcon(":/images/window-new.svg"),"Добавить схему", this);
    ...
    m_removeSchemaButton = new QAction(QIcon(":/images/window-close.svg"),"Удалить схему", this);
    m_removeSchemaButton->setStatusTip("Удаляет схему из рабочей области");
    connect(m_removeSchemaButton, SIGNAL(triggered()), this, SLOT(RemoveScheme()));

    m_cutBlockButton = new QAction(QIcon(":/images/edit-cut.svg"),"Вырезать", this);
    ...
    m_copyBlockButton = new QAction(QIcon(":/images/edit-copy.svg"),"Копировать", this);
    ...
    m_deleteBlockButton = new QAction(QIcon(":/images/edit-delete.svg"),"Удалить",this);
    ...
    m_pasteBlockButton = new QAction(QIcon(":/images/edit-paste.svg"), "Вставить",this);
    ...
    m_propertiesBlockButton = new QAction(QIcon(":/images/preferences-desktop.svg"),"Свойства", this);
    m_propertiesBlockButton->setStatusTip("Параметры блока");
    connect(m_propertiesBlockButton, SIGNAL(triggered()), this, SLOT(PropertiesBlock()));

    m_undoBlockButton = new QAction(QIcon(":/images/edit-undo.svg"), "Отменить",this);
    ...
    m_redoBlockButton = new QAction(QIcon(":/images/edit-redo.svg"), "Повторить",this);
    ...
    m_zoomOutSchemaButton = new QAction(QIcon(":/images/zoom-out.svg"),"Уменьшить Ctrl-", this);
    m_zoomOutSchemaButton->setShortcut(QKeySequence::ZoomOut);
    m_zoomOutSchemaButton->setStatusTip("Уменьшить схему");
    connect(m_zoomOutSchemaButton, SIGNAL(triggered()), this, SLOT(ZoomOutScheme()));

    m_zoomOriginalSchemaButton = new QAction(QIcon(":/images/zoom-original.svg"),"Сбросить зум", this);
    ...
    connect(m_zoomOriginalSchemaButton, SIGNAL(triggered()), this, SLOT(ZoomOriginalScheme()));


    m_zoomInSchemaButton = new QAction(QIcon(":/images/zoom-in.svg"),"Увеличить Ctrl+", this);
    m_zoomInSchemaButton->setShortcut(QKeySequence::ZoomIn);
    m_zoomInSchemaButton->setStatusTip("Увеличить схему");
    connect(m_zoomInSchemaButton, SIGNAL(triggered()), this, SLOT(ZoomInScheme()));

    m_startShemaScriptButton = new QAction(QIcon(":/images/media-playback-start.svg"), "Старт", this);
    m_startShemaScriptButton->setStatusTip("Запустить схему");
    connect(m_startShemaScriptButton, SIGNAL(triggered()), this, SLOT(StartShemaScript()));

    m_pauseShemaScriptButton = new QAction("Пауза", this);
    m_pauseShemaScriptButton->setIcon(style()->standardIcon(QStyle::SP_MediaPause));

    m_stopShemaScriptButton = new QAction(QIcon(":/images/media-playback-stop.svg"),"Стоп", this);
    m_stopShemaScriptButton->setStatusTip("Остановить схему");
    connect(m_stopShemaScriptButton, SIGNAL(triggered()), this, SLOT(StopShemaScript()));

    m_synchronizationTimerBlocksButton = new QAction(QIcon(":/images/timer.png"),"Таймер", this);
    m_synchronizationTimerBlocksButton->setStatusTip("Настройки таймера на схеме");
    connect(m_synchronizationTimerBlocksButton, SIGNAL(triggered()), this, SLOT(ShowTimer()));

    m_selectionPanelElementsButton = new QAction(QIcon(":/images/format-indent-more.svg"),"Добавить элементы на схему", this);
    m_selectionPanelElementsButton->setStatusTip("Добавть блоки на схему");
    connect(m_selectionPanelElementsButton, SIGNAL(triggered()), this, SLOT(ShowSelectPanel()));
    ...
    m_verticalAligning = new QAction(QIcon(":/images/vertical_alig.png"), "Выровнять блоки по вертикали", this);
    m_verticalAligning->setStatusTip("Выровнять блоки по вертикали");
    connect(m_verticalAligning, SIGNAL(triggered()), this, SLOT(VerticalAligning()));

    m_horizontalAligning = new QAction(QIcon(":/images/horizontal_alig.png"), "Выровнять блоки по горизонтали", this);
    m_horizontalAligning->setStatusTip("Выровнять блоки по горизонтали");
    connect(m_horizontalAligning, SIGNAL(triggered()), this, SLOT(HorizontalAligning()));

    m_reloadItemsDialogWidget = new QAction("Перегрузить блоки", this);
    connect(m_reloadItemsDialogWidget, SIGNAL(triggered()), this, SLOT(ReloudBlocks()));
...
void SchemeWindow::CreateCentralWidget()
{
    ...
    //    m_switchCurrentSchema->tabBar()->setTabButton(0, QTabBar::LeftSide, addW);

    m_switchCurrentSchema->tabBar()->connect(m_switchCurrentSchema, SIGNAL(tabBarClicked(int)),
                                             this, SLOT(AddSchemeFromTab(int)));
    connect(m_switchCurrentSchema, SIGNAL(tabBarDoubleClicked(int)), this, SLOT(RenameTabDialog(int)));
    connect(m_switchCurrentSchema, SIGNAL(currentChanged(int)), this, SLOT(UpdateActionsMenu()));
...
bool SchemeWindow::DiagnosisConnectionScheme()
{
    return true;
}
...
QString NameSchemeDialog::GetTextLine() const
{
    return m_textLine->text();
}
...
CloseSchemeDialog::CloseSchemeDialog(QWidget *wparent): QDialog(wparent)
{
    ...
    QGridLayout* layout = new QGridLayout;
    layout->addWidget(new QLabel("Вы действительно хотите завершить работу?"), 0, 0, 1, 2);
