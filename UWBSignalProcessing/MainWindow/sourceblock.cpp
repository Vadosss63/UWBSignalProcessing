...
void SourceBlock::paint(QPainter *painter,
                        const QStyleOptionGraphicsItem *option, QWidget *) {
  DrawRect(painter, option);
}

QVariant SourceBlock::itemChange(QGraphicsItem::GraphicsItemChange change,
...
QVector<OPortBlock *> SourceBlock::GetOutPort() const { return m_port; }
...
void SourceBlock::CreatePorts(QVector<OPort *> ports) {
  ...
  for (uint8_t i = 1; i < outputs + 1; i++) {
    OutPortBlock *outPortBlock = new OutPortBlock(
        QPoint(rect.right(), rect.top() - 6 + i * stepOut), ports[i - 1], this);
