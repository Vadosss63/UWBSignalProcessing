...
AbstractBlock::AbstractBlock(const QString &lable, QGraphicsItem *parent)
    : QGraphicsItem(parent), m_lable(lable), m_textColor(Qt::darkRed),
      m_backgroundColor(Qt::white), m_outLineColor(Qt::darkBlue) {
  setAcceptDrops(true);
  setZValue(0);
...
QPainterPath AbstractBlock::shape() const {
  QPainterPath path;
  path.addRoundedRect(m_borderRect, Roundness(m_borderRect.width()),
                      Roundness(m_borderRect.height()), Qt::RelativeSize);
  return path;
}

void AbstractBlock::paint(QPainter *painter,
...
void AbstractBlock::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  if (acceptDrops()) {
    ...
    QPixmap pixmap(static_cast<int>(boundingRect().height()),
                   static_cast<int>(boundingRect().height()) + 24);
...
void AbstractBlock::SetAbstractDialogCommand(
    DialogWindow *abstractDialogCommand) {
...
QList<std::pair<QString, QVariant>> AbstractBlock::GetSetting() const {
  return m_dialogWindow ? m_dialogWindow->GetSetting()
                        : QList<std::pair<QString, QVariant>>();
}
...
QList<LinkStruct> AbstractBlock::GetLinkStruct() { return QList<LinkStruct>(); }

size_t AbstractBlock::GetId() const { return m_id; }

void AbstractBlock::SetId(size_t id) { m_id = id; }
...
void AbstractBlock::SetRect(QRect &r) { m_borderRect = r; }

void AbstractBlock::SetRectWidth(int w) { m_borderRect.setWidth(w); }

void AbstractBlock::SetRectHeight(int h) { m_borderRect.setHeight(h); }

QRectF &AbstractBlock::GetborderRect() { return m_borderRect; }
...
void AbstractBlock::DrawRect(QPainter *painter,
                             const QStyleOptionGraphicsItem *option) {
  ...
  painter->drawRoundedRect(m_borderRect, Roundness(m_borderRect.width()),
                           Roundness(m_borderRect.height()), Qt::RelativeSize);

  painter->setPen(m_textColor);
  QRectF rectLable(OutlineRect().left(), m_borderRect.bottom(),
                   OutlineRect().width(),
                   OutlineRect().bottom() - m_borderRect.bottom());
...
bool BlockMimeData::HasAbstractBlock() const { return m_block; }

AbstractBlock *BlockMimeData::Block() const { return m_block; }
