...
QVector<BlockStruct> SchemticsController::m_blockStructs = QVector<BlockStruct>();
    ...
    QGraphicsScene(parent), m_time(1), m_dilogTimer(new DilogTimer)
{
    ...
    connect(this, SIGNAL(sceneRectChanged(const QRectF&)), SLOT(update(const QRectF&)));
}
...
void SchemticsController::SetOperationManager(AbstractOperationManager* manager)
{
...
bool SchemticsController::IsStart() const
{
    return m_isStart;
}
...
bool SchemticsController::IsSelecteBlock() const
{
...
bool SchemticsController::IsPaste() const
{
    return !m_blockStructs.isEmpty();
}

bool SchemticsController::IsEmpty() const
{
    return items().empty();
}
...
void SchemticsController::DelBlock()
{
    qDeleteAll(SelectedBlocks());
}
...
void SchemticsController::UnSelecteAll()
{
    clearSelection();
}
...
bool SchemticsController::RenameScheme(const QString& )
{
    return true;
}
...
void TimerRun::Start(uint32_t msec)
{
    m_timer.Start(msec);
}
...
void SchemticsController::CreateLink(const QList<LinkStruct>& connects,  const QMap<size_t, AbstractBlock*>& abstructBlocks)
{
