...
void IOBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
    DrawRect(painter, option);
}

QVariant IOBlock::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant& value)
{
...
QVector<IPortBlock*> IOBlock::GetInPort() const
{
    return m_inPort;
}

QVector<OPortBlock*> IOBlock::GetOutPort() const
{
    return m_outPort;
}
...
void IOBlock::CreateInPortBlock(QVector<IPort*> iports, uint8_t inputs, const QRectF& rect, short stepIn)
{
    for (unsigned short i = 1; i < inputs + 1; i++)
    {
        InPortBlock* inPortBlock = new InPortBlock(QPoint(rect.left() - 24, rect.top()  - 6 + i * stepIn),
                                                   iports[i - 1], this);
        inPortBlock->SetId(i);
...
void IOBlock::CreateOutPortBlock(QVector<OPort*> oports, uint8_t outputs, const QRectF& rect, short stepOut)
{
    for (unsigned short i = 1; i < outputs + 1; i++)
    {
        OutPortBlock* outPortBlock = new OutPortBlock( QPoint(rect.right(), rect.top()  - 6 + i * stepOut),
                                                       oports[i - 1], this);
