...
SchematicsWindow::SchematicsWindow(QWidget *parent) : QMainWindow(parent), m_scene(new SchemticsController(this)),
    m_nodeBuffer(nullptr)
{
    ...
    m_view->setStyleSheet("border: 2px groove gray; border-radius: 10px; padding: 6px 6px;");
    ...
    connect(m_switchShowCloseAction, SIGNAL(triggered()), m_itemDockWidget, SLOT(show()));
    ReadSettings();
...
void SchematicsWindow::Paste()
{
    if(m_nodeBuffer)
    {
        m_scene->AddBlock(m_nodeBuffer->Clone(), QPointF(m_nodeBuffer->pos().x() + 20, m_nodeBuffer->pos().y() + 10));
        UpdateActions();
    }
}
...
void SchematicsWindow::ZoomIn()
{
    m_view->scale(1.5, 1.5);
}

void SchematicsWindow::ZoomOut()
{
...
void SchematicsWindow::Start()
{
    m_scene->Start();
}

void SchematicsWindow::SetIsRelease(bool isRelease)
{
...
void SchematicsWindow::CreateActions()
{
    ...
    m_openAction = new QAction(QIcon(":/Images/document-open.svg"),"Открыть", this);
    m_openAction->setShortcut(QKeySequence::Open);
    m_openAction->setStatusTip("Открыть схему");
    connect(m_openAction, SIGNAL(triggered()), this, SLOT(Open()));

    m_saveAction = new QAction(QIcon(":/Images/document-save.svg"),"Сохранить", this);
    ...
    m_stopAction = new QAction(QIcon(":/Images/media-playback-stop.svg"),"Стоп", this);
    ...
    m_switchShowCloseAction = new QAction(QIcon(":/Images/format-indent-more.svg"),"Добавить элементы на схему", this);
...
void SchematicsWindow::CreateScem(SchemeReader& reader)
{
    ...
    foreach (BlockStruct blockStruct, blockStruct)
    {
        ...
        if(newblock)
        {
            ...
            m_scene->AddBlock(newblock, QPoint(blockStruct.coordX, blockStruct.coordY));
...
bool SchematicsWindow::IsStart() const
{
    return m_isStart;
}

void SchematicsWindow::SetIsStart(bool isStart)
{
    m_isStart = isStart;
}

//ItemsDialogWidget::ItemsDialogWidget(QWidget *parent):QWidget(parent), m_seqNumberItem(0),
...
//    QList<AbstractBlock *> blocks = DatabaseBlocks::Instance()->SourceBlocks();
//    for (auto* blocks : blocks)
//    {
//        SetupItem(blocks->Clone());
//    }
...
//{
//    block->setPos(QPoint(m_marginRectSize.width() + (m_step * (m_seqNumberItem % m_quantityToLineItem)),
//                         m_marginRectSize.height() + (m_step * (m_seqNumberItem / m_quantityToLineItem)))
//                  );
...
//    TextItem* textItem = new TextItem(text, QRectF(QPointF(m_marginRectSize.width(),
//                                                           m_marginRectSize.height() +
//                                                           (m_step * (m_seqNumberItem / m_quantityToLineItem))),
//                                                   m_itemSize));
