...
ItemsDialogWidget::ItemsDialogWidget(QWidget *parent): QWidget(parent),
    m_seqNumberItem(0),
    m_quantityToLineItem(2)
{
    m_scene = new QGraphicsScene(this);
    m_view = new QGraphicsView();
    m_view->setStyleSheet(styleSheet() + "border-radius: 10px; padding: 6px 6px;");
...
void ItemsDialogWidget::SetStyleSheet(QString style)
{
    ...
    rx.setPattern("QGraphicsTextItem.*\\{.*color:\\s*(#[A-fa-f0-9]{6}){1}\\s*;.*\\}");
...
{  
    ...
    int quantityToLineItem = static_cast<int>((widthwin - m_margin)
                                              /(m_rectSize.width() - m_margin));
...
{
    qreal y = m_margin + (m_rectSize.height()) *
              (m_seqNumberItem  / (m_quantityToLineItem));
    return y;
}
