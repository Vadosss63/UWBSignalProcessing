...
SchemaView::SchemaView(const QString &name)
{
    CreateView(name);
}
...
void SchemaView::CreateView(const QString &name)
{
    ...
    m_viewOfSchema->setRenderHints(QPainter::Antialiasing|QPainter::TextAntialiasing);
}
...
void SchemaView::ZoomInScheme()
{
    m_viewOfSchema->scale(1.5, 1.5);
}

void SchemaView::ZoomOriginalScheme()
{
    m_viewOfSchema->resetTransform();
}

void SchemaView::ZoomOutScheme()
{
    m_viewOfSchema->scale(0.75, 0.75);
}
...
void SchemaView::DelBlock()
{
    m_schemaController->DelBlock();
}

void SchemaView::CutBlock()
{
    m_schemaController->CutBlock();
}

void SchemaView::CopyBlock()
{
    m_schemaController->CopyBlock();
}

void SchemaView::PasteBlock()
{
    m_schemaController->PasteBlock();
}

void SchemaView::PropertiesBlock()
{
    m_schemaController->PropertiesBlock();
}

void SchemaView::SelecteAll()
{
    m_schemaController->SelecteAll();
}

void SchemaView::VerticalAligning()
{
    m_schemaController->VerticalAligning();
}
...
void SchemaView::Start()
{
    m_schemaController->Start();
}

void SchemaView::Pause()
{
    m_schemaController->Pause();
}

void SchemaView::Stop()
{
    m_schemaController->Stop();
}

void SchemaView::ShowTimer()
{
    m_schemaController->ShowSettingTimer();
}
...
bool SchemaView::IsStart() const
{
    return m_schemaController->IsStart();
}
...
bool SchemaView::IsPaste() const
{
    return m_schemaController->IsPaste();
}

bool SchemaView::IsEmpty() const
{
    return m_schemaController->IsEmpty();
}

QGraphicsView *SchemaView::GetViewOfSchema() const
{
    return m_viewOfSchema;
}
