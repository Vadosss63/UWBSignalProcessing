...
LineConnect::LineConnect(OPortBlock* fromNode): QGraphicsObject(), m_outPort(fromNode),
    m_inPort(nullptr), m_lines(3), m_borderRect()
{
    SetStatus(outToIn);
    InitLine(fromNode, m_outPort->mapToScene(m_outPort->GetPointLine()));
}

LineConnect::LineConnect(IPortBlock* toNode): QGraphicsObject(), m_outPort(nullptr),
    m_inPort(toNode), m_lines(3), m_borderRect()
{
    SetStatus(inToOut);
...
QPainterPath LineConnect::shape() const
{
    return QPainterPath();
}

void LineConnect::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
...
void LineConnect::TrackNodes(const QPointF &p1, const QPointF &p2)
{
    ...
    if(p1.x() > p2.x())
    {
        if(p1.y() < p2.y())
        {
            m_lines[0].setPoints(p1, QPointF(p1.x(), p1.y() + m_borderRect.height()/2));

            m_lines[1].setPoints(QPointF(p1.x(), p1.y() + m_borderRect.height()/2),
                                 QPointF(p2.x(), p1.y() + m_borderRect.height()/2));

            m_lines[2].setPoints(QPointF(p2.x(), p1.y() + m_borderRect.height()/2), p2);

            m_borderRect = QRectF(QPointF(p2.x(), p1.y()),
                                  QPointF(p1.x(), p2.y()));
        }
        else
        {

            m_lines[0].setPoints(p1, QPointF(p1.x(), p2.y() + m_borderRect.height()/2));

            m_lines[1].setPoints(QPointF(p1.x(), p2.y() + m_borderRect.height()/2),
                                 QPointF(p2.x(), p2.y() + m_borderRect.height()/2));
            m_lines[2].setPoints(QPointF(p2.x(), p2.y() + m_borderRect.height()/2), p2);
    ...
    {
        m_lines[0].setPoints(p1, QPointF(p1.x() + m_borderRect.width()/2, p1.y()));

        m_lines[1].setPoints(QPointF(p1.x() + m_borderRect.width()/2, p1.y()),
                             QPointF(p1.x() + m_borderRect.width()/2, p2.y()));

        m_lines[2].setPoints(QPointF(p1.x() + m_borderRect.width()/2, p2.y()), p2);

        if(p1.y() > p2.y())
            m_borderRect = QRectF(QPointF(p1.x(), p2.y()),
                                  QPointF(p2.x(), p1.y()));
        else
...
void LineConnect::SetPen(const QPen &pen)
{
    m_pen = pen;
}
...
OPortBlock* LineConnect::GetOutPort() const
{
    return m_outPort;
}

IPortBlock* LineConnect::GetInPort() const
{
    return m_inPort;
}
...
{
    TrackNodes(m_outPort->mapToScene(m_outPort->GetPointLine()), event->scenePos());
    if(scene()->itemAt(event->scenePos(), QTransform()))
    {
        IPortBlock* other = dynamic_cast<IPortBlock*>(scene()->itemAt(event->scenePos(), QTransform()));
...
void LineConnect::HoverEnterOutPort(QGraphicsSceneMouseEvent *event)
{
    ...
    if(scene()->itemAt(event->scenePos(), QTransform()))
    {
        OPortBlock* other = dynamic_cast<OPortBlock*>(scene()->itemAt(event->scenePos(), QTransform()));
        if(other)
            if(!other->GetLine())
            {
                other->CreateImageHoverEnter();
...
statusConnect LineConnect::GetStatus()
{
...
void LineConnect::SetStatus(statusConnect status)
{
    ...
    {
    case outToIn:
        connect(this, SIGNAL(MouseMove(QGraphicsSceneMouseEvent*)), SLOT(HoverEnterInPort(QGraphicsSceneMouseEvent*)));
        break;
    case inToOut:
        connect(this, SIGNAL(MouseMove(QGraphicsSceneMouseEvent*)), SLOT(HoverEnterOutPort(QGraphicsSceneMouseEvent*)));
        break;
    case connects:
    {
        if(m_flagConnect == outToIn)
            disconnect(this, SIGNAL(MouseMove(QGraphicsSceneMouseEvent*)),this, SLOT(HoverEnterInPort(QGraphicsSceneMouseEvent*)));
        if(m_flagConnect == inToOut)
            disconnect(this, SIGNAL(MouseMove(QGraphicsSceneMouseEvent*)),this, SLOT(HoverEnterOutPort(QGraphicsSceneMouseEvent*)));
    }
        break;
    }
...
AbstractPortBlock::AbstractPortBlock(const QPoint& point, QGraphicsItem* parent): QGraphicsObject(parent),
    m_lines(nullptr), sizeEllipse(1)
{
    setZValue(1);
...
AbstractPortBlock::~AbstractPortBlock()
{
    delete m_lines;
}

QRectF AbstractPortBlock::boundingRect() const
{
    return m_borderRect;
}

void AbstractPortBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    DrawRect(painter);
}
...
void AbstractPortBlock::ConnectPorts(OPortBlock* outPortBlock, IPortBlock* inPortBlock)
{
...
size_t AbstractPortBlock::GetId() const
{
    return m_id;
}

void AbstractPortBlock::SetId(size_t id)
{
    m_id = id;
}

void AbstractPortBlock::SetParentId(size_t parentId)
{
    m_parentId = parentId;
}

size_t AbstractPortBlock::GetParentId() const
{
    return m_parentId;
}
...
void AbstractPortBlock::LineReset()
{
    m_lines = nullptr;
}
...
void AbstractPortBlock::SetLine(LineConnect *Lines)
{
    m_lines = Lines;
}

LineConnect* AbstractPortBlock::GetLine() const
{
    return m_lines;
}
...
InPortBlock::InPortBlock(const QPoint& point, IPort* inputPort, QGraphicsItem* parent)
    : IPortBlock(point, parent),
      m_inputPort(inputPort)
{
    QRectF borderRect = boundingRect();
    inPuts = QLine(static_cast<int>(borderRect.left()) + 6, static_cast<int>(borderRect.top() + borderRect.height()/2),
                   static_cast<int>(borderRect.right()), static_cast<int>(borderRect.top() + borderRect.height()/2));
}
...
IPort*InPortBlock::GetInputPort()
{
    return m_inputPort;
}
...
void InPortBlock::TryConnect()
{
    ...
    OPortBlock* oPort = dynamic_cast<OPortBlock*>(scene()->itemAt(mapToScene(GetPointLine()), QTransform()));
    if(oPort)
    {
        if(!oPort->GetLine())
        {
...
void InPortBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    DrawRect(painter);
}
...
OutPortBlock::OutPortBlock(const QPoint& point, OPort* outputPort, QGraphicsItem* parent)
    : OPortBlock(point, parent),
      m_outputPort(outputPort)
{
    QRectF borderRect = boundingRect();
    outPuts = QLine(static_cast<int>(borderRect.left()), static_cast<int>(borderRect.top() + borderRect.height()/2),
                    static_cast<int>(borderRect.right()) - 6, static_cast<int>(borderRect.top() + borderRect.height()/2));
}
...
OPort* OutPortBlock::GetOutputPort()
{
    return m_outputPort;
}
...
void OutPortBlock::TryConnect()
{
    ...
    IPortBlock* iPort = dynamic_cast<IPortBlock*>(scene()->itemAt(mapToScene(GetPointLine()), QTransform()));
    if(iPort)
    {
        if(!iPort->GetLine())
        {
...
void OutPortBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    DrawRect(painter);
}
