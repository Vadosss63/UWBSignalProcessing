...
void PtreeWR::AddParameterField(const std::string &pathValue,
...
QMap<size_t, BlockStruct> PtreeWR::GetBlocks() const {
  ...
  if (blocks) {
    for (auto &el : *blocks) {
      ...
      blockStruct.id =
          QString::fromStdString(el.second.get<std::string>(m_textID)).toUInt();
      blockStruct.type =
          QString::fromStdString(el.second.get<std::string>(m_textType));
      blockStruct.coordX =
          QString::fromStdString(el.second.get<std::string>(m_textCoordX))
              .toInt();
      blockStruct.coordY =
          QString::fromStdString(el.second.get<std::string>(m_textCoordY))
              .toInt();
      auto parametres = m_treeJson.get_child_optional(
          std::string(m_textBlock) + "." +
          el.second.get<std::string>(m_textID) + "." +
          std::string(m_textParametres) + ".");
      ...
      if (parametres) {
        for (auto &param : *parametres) {
          ...
          //                    boost::property_tree::write_json(std::cerr,
          //                    parametres.value());

          listParam.push_back(std::make_pair(
              QString::fromStdString(parametrPath),
...
QList<LinkStruct> PtreeWR::GetLinks() const {
  ...
  if (links) {
    for (auto &el : *links) {
      ...
      linkStruct.idInBlock =
          QString::fromStdString(el.second.get<std::string>(m_textBlockIn))
              .toUInt();
      linkStruct.idOutBlock =
          QString::fromStdString(el.second.get<std::string>(m_textBlockOut))
              .toUInt();
      linkStruct.idInPort =
          QString::fromStdString(el.second.get<std::string>(m_textNumberPortIn))
              .toUInt();
      linkStruct.idOutPort = QString::fromStdString(el.second.get<std::string>(
                                                        m_textNumberPortOut))
                                 .toUInt();
