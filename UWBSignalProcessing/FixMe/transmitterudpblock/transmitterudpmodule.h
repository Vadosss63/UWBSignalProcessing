...
// TransmitterUDPModule включает в себя ServerUDP, который в свою очередь является оберткой
// для boost::asio
// Данный клас выполняет передачу данных типа csignal путем преобразование его в arry массив
// в конструкторе ServerUDP выполняется иницилизация ip адреса и порта протокола UDP
