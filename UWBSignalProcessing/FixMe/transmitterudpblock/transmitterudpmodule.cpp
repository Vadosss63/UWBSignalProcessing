...
SenderUDPModule::~SenderUDPModule() { StopServer(); }
...
void SenderUDPModule::CreateProtocol(csignal<double> &sig) {
  ...
  for (uint8_t i = 0; i < size; ++i) {
    ...
    std::copy(sig.begin() + Datagram::DatagramPRLI::maxSize * i,
              sig.begin() +
                  (Datagram::DatagramPRLI::maxSize * i + sigData.size()),
              sigData.begin());
    ...
    // Пелинг
    data.dataByte.prli.bearing =
        qRound(sigData.azimuth() * std::pow(2, 16) / 360.0 * 90 / 64);
    // Угол места
    ...
    dspm::csignal<unsigned char> ucSig =
        static_cast<dspm::csignal<unsigned char>>(sigData);
...
void SenderUDPModule::StartOperation() { StartServer(); }
