...
//Класс ReceiverUDPModule  выполяет обработку  сигнала, принятого по протоколу UDP
...
public:
    class ControlCommand : public std::function<void(void)>
    {
        ...
        ControlCommand(ReceiverUDPModule* parent, size_t coef)
            : std::function<void(void)>(std::bind(&ReceiverUDPModule::SetCoefRecive, parent, coef))
        { }
    };
