...
/// \param parent
///
PRLIClient::PRLIClient(QObject *parent)
    : Network::ClientUDP("224.1.1.30", 22251,
                         sizeof(Datagram::DatagramPRLI::PRLI) +
...
void PRLIClient::SetCoef(size_t coef) { m_coef = coef; }
...
void PRLIClient::PushCollection(
    DataParser::Parser<Datagram::DatagramPRLI::PRLI> &parser) {
  ...
  temp.set_azimuth(
      qRound(package.infoBody().bearing * 360.0 / std::pow(2, 16) * 64 / 90));
  ...
  if (m_collection.size() == package.header().numberOfInformationPackets) {
    ...
    for (size_t i = 1; i < m_collection.size() + 1; ++i)
      for (size_t j = 0; j < m_collection[i].size() / coef; ++j)
        temp[j + (i - 1) * m_collection[i].size() / coef] =
            m_collection[i][j * coef];
...
ReceiverUDPModule::ReceiverUDPModule(QObject *parent)
    : SourceModule(static_cast<ports_quantity>(1), parent), m_client(this),
      m_frame(0) {
...
void ReceiverUDPModule::StartOperation() { StartThread(); }
...
void ReceiverUDPModule::SetCoefRecive(size_t coef) { emit SetCoef(coef); }
