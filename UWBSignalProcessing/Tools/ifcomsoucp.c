...
void write_bitfield_uint16_t_rl(ifcomsoucp_DataStream *data, uint16_t value, size_t bitsCount);
uint16_t read_bitfield_uint16_t_rl(ifcomsoucp_DataStream *data, size_t bitsCount);
...
void write_bitfield_uint8_t_rl(ifcomsoucp_DataStream *data, uint8_t value, size_t bitsCount);
...
void write_bitfield_uint32_t_rl(ifcomsoucp_DataStream *data, uint32_t value, size_t bitsCount);
uint32_t read_bitfield_uint32_t_rl(ifcomsoucp_DataStream *data, size_t bitsCount);
...
void ifcomsoucp_init(ifcomsoucp_Status *status, uint8_t *data, size_t dataLength, ifcomsoucp_Mode mode)
{
...
void ifcomsoucp_reset(ifcomsoucp_Status *status, uint8_t *data, size_t dataLength, ifcomsoucp_Mode mode)
{
...
void ifcomsoucp_resetDataStream(ifcomsoucp_Status *status, uint8_t *data, size_t dataLength)
{
...
int ifcomsoucp_updateHeader_words32Count(ifcomsoucp_Status *status, uint16_t value)
{
...
ifcomsoucp_CRPlan_Object *ifcomsoucp_getCRPlan_Object(ifcomsoucp_Status *status)
{
    return (ifcomsoucp_CRPlan_Object *)status->base.structPtr;
}

ifcomsoucp_DestroyObject *ifcomsoucp_getDestroyObject(ifcomsoucp_Status *status)
{
    return (ifcomsoucp_DestroyObject *)status->base.structPtr;
}

void ifcomsoucp_setStructID(ifcomsoucp_Status *status, ifcomsoucp_StructID structID)
{
...
int ifcomsoucp_writeStruct(ifcomsoucp_Status *status)
{
    ...
    int stackPtr = 0;
    while(stackPtr != -1) switch(stack[stackPtr--])
    {
    case 1:
        stack[++stackPtr] = 2;
    ...
    case 3:
        stack[++stackPtr] = 4;
        if(!(status->base.Header_messageType == IFCOMSOUCP_MESSAGETYPE_OBJECT_INFO))
            stack[stackPtr] = 11;
        else
    ...
    case 4:
        if(status->base.structID != IFCOMSOUCP_STRUCT_ID_OBJECTINFO)
            continue;
        if(write_ObjectInfo(status, (ifcomsoucp_ObjectInfo*)status->base.structPtr)) 
        {
            status->base.state = 5;
            status->backup = status->base;
            return 1;
    ...
    case 8:
        stack[++stackPtr] = 9;
        if(!(status->base.ObjectInfo_typeExtData == IFCOMSOUCP_TYPEEXTDATA_METEO))
            stack[stackPtr] = ifcomsoucp_pop(status);
        continue;
    case 9:
        stack[++stackPtr] = 10;
    ...
    case 13:
        stack[++stackPtr] = 14;
        if(!(status->base.Control_type == IFCOMSOUCP_CONTROLMESSAGETYPE_VZOI_VZOR_SETTINGS))
            stack[stackPtr] = ifcomsoucp_pop(status);
        continue;
    case 14:
        if(status->base.structID != IFCOMSOUCP_STRUCT_ID_VZOIVZORDATA)
            continue;
        if(write_VzoiVzorData(status, (ifcomsoucp_VzoiVzorData*)status->base.structPtr)) 
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
            return 1;
    ...
    case 19:
        stack[++stackPtr] = 20;
        if(!(status->base.Control_type == IFCOMSOUCP_CONTROLMESSAGETYPE_CR_PLAN_POINT))
            stack[stackPtr] = 21;
        else
    ...
    case 20:
        if(status->base.structID != IFCOMSOUCP_STRUCT_ID_CRPLAN_POINT)
            continue;
        if(write_CRPlan_Point(status, (ifcomsoucp_CRPlan_Point*)status->base.structPtr)) 
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
            return 1;
    ...
    case 21:
        stack[++stackPtr] = 22;
        if(!(status->base.Control_type == IFCOMSOUCP_CONTROLMESSAGETYPE_CR_PLAN_OBJECT))
            stack[stackPtr] = 23;
        else
    ...
    case 22:
        if(status->base.structID != IFCOMSOUCP_STRUCT_ID_CRPLAN_OBJECT)
            continue;
        if(write_CRPlan_Object(status, (ifcomsoucp_CRPlan_Object*)status->base.structPtr)) 
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
            return 1;
    ...
    case 23:
        stack[++stackPtr] = 24;
        if(!(status->base.Control_type == IFCOMSOUCP_CONTROLMESSAGETYPE_DESTROY_OBJECT))
            stack[stackPtr] = ifcomsoucp_pop(status);
        continue;
    case 24:
        if(status->base.structID != IFCOMSOUCP_STRUCT_ID_DESTROYOBJECT)
            continue;
        if(write_DestroyObject(status, (ifcomsoucp_DestroyObject*)status->base.structPtr)) 
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
            return 1;
...
int ifcomsoucp_readStruct(ifcomsoucp_Status *status)
{
    ...
    case 3:
        ...
        continue;
    case 4:
        if(read_ObjectInfo(status, (ifcomsoucp_ObjectInfo*)status->base.structPtr))
        {
            status->base.state = 5;
            status->backup = status->base;
    ...
    case 13:
        ...
        continue;
    case 14:
        if(read_VzoiVzorData(status, (ifcomsoucp_VzoiVzorData*)status->base.structPtr))
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
    ...
    case 19:
        ...
        continue;
    case 20:
        if(read_CRPlan_Point(status, (ifcomsoucp_CRPlan_Point*)status->base.structPtr))
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
    ...
    case 21:
        ...
        continue;
    case 22:
        if(read_CRPlan_Object(status, (ifcomsoucp_CRPlan_Object*)status->base.structPtr))
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
    ...
    case 23:
        ...
        continue;
    case 24:
        if(read_DestroyObject(status, (ifcomsoucp_DestroyObject*)status->base.structPtr))
        {
            status->base.state = ifcomsoucp_pop(status);
            status->backup = status->base;
