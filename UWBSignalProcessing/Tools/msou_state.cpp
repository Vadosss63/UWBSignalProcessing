    ...
    if (index < m_amount)
    {
        ...
        if(state)
        {
            if(index == m_amount - 1)
            {
                ...
                if(!endButtonState) // устанавливаем желтый цвет, если какой-то из приборов не работает
                {
...
MSOU_State::MSOU_State(QWidget *parent)
    : StateButton(QVector<QString>() << "СИ.01" << "СИ.02" << "ЦП.01" << "ЦП.02" << "ЦП.03"
                                     << "ПУР-Х" << "ЭМС" << "МА4.1" << "МА4.2" << "МА5.1"
                                     <<  "МА5.2" << "МА5.3" << "МА5.4" << "МСОУ", parent){}
