...
QColor DrawCurver::m_colorForIds[]{Qt::white,   Qt::red,  Qt::yellow,
                                   Qt::magenta, Qt::cyan, Qt::blue};
...
QSize DrawCurver::sizeHint() const { return QSize(12 * MARGIN, 8 * MARGIN); }
...
void DrawCurver::SetColorGrid(QColor color) { m_colorGrid = std::move(color); }
...
void DrawCurver::drawGrig(QPainter *painter) {
  ...
  painter->drawText(rect.center().x() - 50, rect.bottom() + 30, 100, 20,
                    Qt::AlignHCenter | Qt::AlignTop, lableXText);
...
size_t DrawCurver::GetSizeData() { return m_curves.size(); }
...
PlotSetting::PlotSetting(double max_X, double max_Y, double num_XTicks,
                         double num_YTicks) {
  minX = 0;
  maxX = max_X;
...
PlotSetting::PlotSetting(double max_X, double max_Y, double min_X, double min_Y,
                         double num_XTicks, double num_YTicks) {
  minX = min_X;
  maxX = max_X;
  numXTicks = num_XTicks;
  minY = min_Y;
...
Plotter::Plotter(CSignalPlotter *scopeBufferModule, QWidget *parent)
    : DrawCurver(parent), m_scopeBufferModule(scopeBufferModule) {
...
PlotterComplex::PlotterComplex(CSignalComplexPlotter *scopeBufferModule,
                               QWidget *parent)
    : DrawCurver(parent, 2), m_scopeBufferModule(scopeBufferModule) {
  connect(scopeBufferModule, SIGNAL(BufferUpdated()), this, SLOT(CreateData()));
}
