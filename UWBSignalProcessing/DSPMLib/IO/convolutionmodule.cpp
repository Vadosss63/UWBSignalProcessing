...
dspm::ConvolutionModule::ConvolutionModule(const int &fft_length):    
      m_tempFFT(fft_length),
      m_tempFFT_filter(fft_length),
      m_sigTemp(fft_length),
      m_filterTemp(fft_length),
      m_fft_length(fft_length)
{

    //Создание плана для прямого преобразования Фурье сигнала
    m_plan_fft_forward_signal = fftw_plan_dft_r2c_1d(m_tempFFT.size(), (double*) &m_sigTemp[0], (fftw_complex*) &m_tempFFT[0], FFTW_ESTIMATE );
    //Создание плана для прямого преобразования Фурье фильтра
    m_plan_fft_forward_filter = fftw_plan_dft_r2c_1d(m_tempFFT_filter.size(), (double*) &m_filterTemp[0], (fftw_complex*) &m_tempFFT_filter[0], FFTW_ESTIMATE );
    //Создание плана для обратноог преобразования Фурье
    m_plan_fft_backward = fftw_plan_dft_c2r_1d (m_tempFFT.size(), (fftw_complex*) &m_tempFFT[0], (double*) &m_sigTemp[0], FFTW_ESTIMATE );
}
...
void dspm::ConvolutionModule::LetConv(csignal<double> &in, const std::vector<double>& filter )
{
    ...
    std::transform(m_tempFFT.begin(), m_tempFFT.end(), m_tempFFT_filter.begin(), m_tempFFT.begin(),
    ...
    auto res_time = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time-start_time);
    qDebug()<<res_time.count();


}

void dspm::ConvolutionModule::LetConv(csignal<double> &in1, csignal<double> &in2, const std::vector<double> &filter)
{
    std::vector<double>temp;
...
void dspm::ConvolutionModule::LetConv(std::vector<double> &in, const std::vector<double> &filter)
{
    std::vector<double>temp;
    temp = filter;
...
dspm::csignal<double> dspm::ConvolutionModule::LetSimpConv(csignal<double> &in, const std::vector<double> &filter)
{
    auto start_time = std::chrono::steady_clock::now();
