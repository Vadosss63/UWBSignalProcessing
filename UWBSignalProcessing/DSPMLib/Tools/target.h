...
//для чтения из файла
template<typename T>
Target& operator >> (T& in, Target& target)
{
    ...
    for (int i = 0; i < countPoint; ++i) {
        ...
        in.read(reinterpret_cast<char*>(&(point.distence)), sizeof(point.distence));
        in.read(reinterpret_cast<char*>(&(point.direction)), sizeof(point.direction));
...
void operator << (T& out, const Target& target)
{
    ...
    for (const Target::Point& point : listPoints) {
        out.write(reinterpret_cast<char const*>(&(point.speed)), sizeof(point.speed));
        out.write(reinterpret_cast<char const*>(&(point.distence)), sizeof(point.distence));
        out.write(reinterpret_cast<char const*>(&(point.direction)), sizeof(point.direction));
