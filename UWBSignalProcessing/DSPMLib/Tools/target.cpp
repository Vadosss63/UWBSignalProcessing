...
double Target::GetToSpeedKMH(size_t s) const { return 3.6 * GetSpeedMS(s); }
...
size_t Target::GetCountAcumm() const { return m_countAcumm; }

void Target::SetCountAcumm(const size_t &countAcumm) {
  m_countAcumm = countAcumm;
}

size_t Target::GetMaxSpeed() { return m_maxSpeed; }

void Target::SetMaxSpeed(const size_t &maxSpeed) { m_maxSpeed = maxSpeed; }
...
size_t Target::Size() const { return m_pointTarget.size(); }

size_t Target::Size() { return m_pointTarget.size(); }

std::list<Target::Point> &Target::GetPoints() { return m_pointTarget; }

const std::list<Target::Point> &Target::GetPoints() const {
  return m_pointTarget;
}

Target::Point &Target::GetFont() { return m_pointTarget.front(); }
