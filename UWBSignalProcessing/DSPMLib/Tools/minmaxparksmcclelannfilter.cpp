...
MinMaxParksMcClelannFilter::MinMaxParksMcClelannFilter(const int& num_taps, const int& n_bands,std::vector<double> &bands_edge, const std::vector<double>& desire,const std::vector<double> &weight, const FilterType& type, const int& grid_density, const int& max_iterations)
    :h(num_taps)
{

    remez(num_taps,n_bands,bands_edge,desire,weight,type,grid_density,max_iterations);
}
...
int MinMaxParksMcClelannFilter::GetRemezStatus()
{
        return status;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void MinMaxParksMcClelannFilter::CreateDenseGrid(const int &numtaps, const int &numband,  std::vector<double> &bands, const std::vector<double> &des, const std::vector<double> &weight,const int &grid_density)
{
    int j, k;

    double delf = 0.5 / (grid_density*r);

    ///Фильтры типа Гильберта и дифференциатора имеют нечетную симметрю, и по этому: grid[0] = max(delf, band[0])
    ...
    for (int band_index = 0; band_index < numband; band_index++)
    {
        ...
        k = static_cast<int>(((highf - lowf) / delf) + 0.5); //0.5 для округления (зачем? ПРОВЕРИТЬ)!!!!
    ...
    ///Согласно тому, что говорилось выше про нечетную симметрию фильтров. Последней точкой в сетке не может быть частота Найквиста (0.5), но если у нас четная длина фильтра, то точку можно оставить

    if ((remez_symmetry == 0) && (grid.at(gridsize - 1) > (0.5 - delf)) && (numtaps % 2))
    {
...
void MinMaxParksMcClelannFilter::CalcParams()
{
       ...
       int ld = (r - 1) / 15 + 1;
        ...
        //Рассчитываем дельту  - Oppenheim & Schafer eq 7.131 (Отклонение частотной характеристики)
...
void MinMaxParksMcClelannFilter::Search()
{
    ...
    int i, j, k, l, extra; //Счетчики
        ...
        if (((e.at(0) > 0.0) && (e.at(0) > e.at(1))) || ((e.at(0) < 0.0) && (e.at(0) < e.at(1))))
            foundExt.at(k++) = 0;

        //Проверяем на экстремумы внутри густой сетки

        for (i = 1; i < gridsize - 1; i++) {
            if (((e.at(i) >= e.at(i - 1)) && (e.at(i) > e.at(i + 1)) && (e.at(i) > 0.0))
                    || ((e.at(i) <= e.at(i-1)) && (e.at(i) < e.at(i+1)) && (e.at(i) < 0.0)))
                foundExt.at(k++) = i;
        }
        ...
        while (extra > 0) {
            ...
            //Если у нас один экстремум и все они противоположны, удаляем наименьший первый/последний
...
////////////////////////////////////////////////////////////////////////////////////
void MinMaxParksMcClelannFilter::FreqSample(const int& N, std::vector<double> A)
{
    int n, k;
        double x, val, M;
...
////////////////////////////////////////////////////////////////////////////////////////////////
void MinMaxParksMcClelannFilter::remez(const int &num_taps, const int &n_bands,std::vector<double> &bands_edge, const std::vector<double> &desire, const std::vector<double> &weight, const MinMaxParksMcClelannFilter::FilterType &type, const int &grid_density, const int &max_iterations)
{
   // double *mem;
        int i, iter; //*imem;
        double c;
        ...
        gridsize = 0;
        for (i = 0; i < n_bands; i++) {
            gridsize += static_cast<int>(2 * r * grid_density
                    * (bands_edge.at(2 * i + 1) - bands_edge.at(2 * i)) + .5f); ////////////!!!!!! КРИВЫЕ КАСТЫ
        }
        if (!remez_symmetry) {
            gridsize--;
        }
...
//        }
        ...
        //В оригинальном алгоритме выделение памяти велось посредством нарезания кусков сырого пула памяти
