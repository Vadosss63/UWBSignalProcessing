...
std::vector<std::complex<double>> time_to_freq(const std::vector<double> &sig) {
  ...
  fftw_plan m_plan_fft_forward_signal = fftw_plan_dft_r2c_1d(
      static_cast<int>(sig_size), reinterpret_cast<double *>(&tmp[0]),
      reinterpret_cast<fftw_complex *>(&spectrum_signal[0]), FFTW_ESTIMATE);
...
std::vector<double>
freq_to_time(const std::vector<std::complex<double>> &input_sig) {
  ...
  fftw_plan m_plan_fft_backward =
      fftw_plan_dft_c2r_1d(sig_size, (fftw_complex *)&input_sig[0],
                           (double *)&tmp[0], FFTW_ESTIMATE);

  fftw_execute(m_plan_fft_backward);

  double N = 1.0 / sig_size;

  std::for_each(tmp.begin(), tmp.end(), [&N](double &h) { h *= N; });
...
conj(const std::vector<std::complex<double>> &sig) {
  auto tmp(sig);

  std::for_each(tmp.begin(), tmp.end(), [](std::complex<double> &h) {
    h = std::conj(h);
    ;
  });
...
std::vector<double> conv(const std::vector<double> &sig1,
  ...
  std::transform(spectr1.begin(), spectr1.end(), spectr2.begin(),
                 spectr3.begin(),
