...
template <> csignal<double>::operator csignal<unsigned char>() {
  ...
  std::transform(std::begin(this->m_signal), std::end(this->m_signal),
                 temp.begin(), [](double i) {
                   if (i < 0)
                     return 0;
                   if (i > 2047)
...
//     std::transform(std::begin(this->m_signal), std::end(this->m_signal),
//     temp.begin(),
//                    [](unsigned char i) { return (static_cast<double>(i <<
//                    3)); });
//     return temp;
// }
...
csignal<double> AbsComplex(const csignal<double> &reSig,
  ...
  if (reSig.size() == imSig.size()) {
    ...
    std::transform(reSig.begin(), reSig.end(), imSig.begin(), temp.begin(),
                   [](double reVal, double imVal) {
                     return std::abs(std::complex<double>(reVal, imVal));
                   });
    return temp;
  }
...
csignal<double> ArgComplex(const csignal<double> &reSig,
  ...
  if (reSig.size() == imSig.size()) {
    ...
    std::transform(reSig.begin(), reSig.end(), imSig.begin(), temp.begin(),
                   [](double reVal, double imVal) {
                     return std::arg(std::complex<double>(reVal, imVal));
                   });
    return temp;
  }
...
csignal<double> ComplexIm(const csignal<double> &absSig,
  ...
  if (absSig.size() == argSig.size()) {
    ...
    std::transform(absSig.begin(), absSig.end(), argSig.begin(), temp.begin(),
                   [](double absVal, double argVal) {
                     return std::polar(absVal, argVal).imag();
                   });
    return temp;
  }
...
csignal<double> ComplexRe(const csignal<double> &absSig,
  ...
  if (absSig.size() == argSig.size()) {
    ...
    std::transform(absSig.begin(), absSig.end(), argSig.begin(), temp.begin(),
                   [](double absVal, double argVal) {
                     return std::polar(absVal, argVal).real();
                   });
    return temp;
  }
