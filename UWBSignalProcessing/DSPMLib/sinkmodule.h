...
class SinkModule : public AbstractModule
{

public:

    //конструктор выделяет память для ModuleInput, указывая себя в качестве его родителя
...
template<typename BufferTypeIn>
SinkModule<BufferTypeIn>::~SinkModule()
{
...
{
    if (std::all_of(inputPorts.begin(), inputPorts.end(), [](const InputPort<BufferTypeIn>* ip){ return !ip->Empty(); }))
        AbstractModule::QueueOperation();
}

template<typename BufferTypeIn>
void SinkModule<BufferTypeIn>::StopEvent()
{
    std::for_each(inputPorts.begin(), inputPorts.end(), [](InputPort<BufferTypeIn>* ip){ ip->ClearDataQueue(); });
}

template<typename BufferTypeIn>
bool SinkModule<BufferTypeIn>::PopFromInput(BufferTypeIn& element, uint8_t inputNo)
{
