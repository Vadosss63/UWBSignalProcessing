...
template <class T> class csignal {
public:
  //////////////////////////////////////////////////////////////////
  ...
  friend void operator<<(Tstreem &out, const csignal<U> &signal);
  // для чтения из файла
  template <typename U, typename Tstreem>
  friend csignal<U> &operator>>(Tstreem &in, csignal<U> &signal);
...
template <class T> void csignal<T>::apply(T func(T)) {
  m_signal = m_signal.apply(func);
}

template <class T> uint32_t csignal<T>::m_simpleSizeAzimuth = 64;

/////////////////////////////////////////////////////////////////////
/// описание методов класса
/////////////////////////////////////////////////////////////////////

template <class T> csignal<T>::csignal() {}

template <class T> csignal<T>::csignal(size_t size) : m_signal(size) {}

template <class T>
csignal<T>::csignal(size_t size, const T &element) : m_signal(element, size) {}

template <class T>
csignal<T>::csignal(const csignal &rhs)
    : m_signal(rhs.m_signal), m_azimuth(rhs.m_azimuth), m_range(rhs.m_range),
      m_frameNumber(rhs.m_frameNumber) {}

template <class T>
csignal<T>::csignal(csignal &&rhs)
    : m_signal(std::move(rhs.m_signal)), m_azimuth(rhs.m_azimuth),
      m_range(rhs.m_range), m_frameNumber(rhs.m_frameNumber) {}

template <class T> csignal<T>::~csignal() {}

template <class T> csignal<T> &csignal<T>::operator=(const csignal &rhs) {
  if (this != &rhs) {
...
template <class T> csignal<T> &csignal<T>::operator=(csignal &&rhs) {
  if (this != &rhs) {
...
template <class T> T &csignal<T>::operator[](size_t index) {
  return m_signal[index];
}

template <class T> const T &csignal<T>::operator[](size_t index) const {
  return m_signal[index];
}
...
template <class T> csignal<T> &csignal<T>::operator+=(const T &val) {
  m_signal += val;
  return *this;
}
...
template <class T> csignal<T> &csignal<T>::operator*=(double mult) {
  m_signal *= mult;
  return *this;
}

template <class T> csignal<T> &csignal<T>::operator/=(size_t divide) {
  m_signal /= divide;
  return *this;
}
...
template <class T> bool csignal<T>::operator<(const T &thresh) {
  return std::any_of(std::begin(m_signal), std::end(m_signal),
                     [thresh](const T &element) { return element < thresh; });
}

template <class T> bool csignal<T>::operator<(const T &thresh) const {
  return std::any_of(std::begin(m_signal), std::end(m_signal),
                     [thresh](const T &element) { return element < thresh; });
}
...
template <class T> bool csignal<T>::operator==(const csignal<T> &signalOther) {
  if (m_signal.size() != signalOther.m_signal.size())
    return false;
  if (m_azimuth != signalOther.m_azimuth)
    return false;
  if (m_frameNumber != signalOther.m_frameNumber)
    return false;
...
template <class T> size_t csignal<T>::size() const { return m_signal.size(); }

template <class T> bool csignal<T>::empty() const { return !m_signal.size(); }

template <class T> void csignal<T>::reset(size_t new_size) {
  m_signal.resize(new_size);
}

template <class T> void csignal<T>::clear() { m_signal.resize(0); }

template <class T> uint32_t csignal<T>::azimuth() const { return m_azimuth; }

template <class T> uint8_t csignal<T>::range() const { return m_range; }

template <class T> uint16_t csignal<T>::frameNumber() const {
  return m_frameNumber;
}

template <class T> uint32_t csignal<T>::simpleSizeAzimuth() const {
  return m_simpleSizeAzimuth;
}

template <class T> void csignal<T>::setAzimuth(uint32_t az) {
  m_azimuth = az % simpleSizeAzimuth();
}

template <class T> void csignal<T>::setRange(uint8_t ran) { m_range = ran; }

template <class T> void csignal<T>::setFrameNumber(uint16_t frame) {
  m_frameNumber = frame;
}
...
template <class T> void csignal<T>::abs() { m_signal = std::abs(m_signal); }

template <class T> void csignal<T>::exp() { m_signal = std::exp(m_signal); }

template <class T> void csignal<T>::sqrt() { m_signal = std::sqrt(m_signal); }

template <class T> void csignal<T>::log() { m_signal = std::log(m_signal); }

template <class T> void csignal<T>::log10() { m_signal = std::log10(m_signal); }

template <class T> void csignal<T>::sin() { m_signal = std::sin(m_signal); }

template <class T> void csignal<T>::cos() { m_signal = std::cos(m_signal); }

template <class T> void csignal<T>::tan() { m_signal = std::tan(m_signal); }

template <class T> void csignal<T>::sinh() { m_signal = std::sinh(m_signal); }

template <class T> void csignal<T>::cosh() { m_signal = std::cosh(m_signal); }

template <class T> void csignal<T>::tanh() { m_signal = std::tanh(m_signal); }

template <class T> void csignal<T>::asin() { m_signal = std::asin(m_signal); }

template <class T> void csignal<T>::acos() { m_signal = std::acos(m_signal); }

template <class T> void csignal<T>::atan() { m_signal = std::atan(m_signal); }

template <class T> void csignal<T>::pow(int val) {
  m_signal = std::pow(m_signal, static_cast<T>(val));
}

template <class T> void csignal<T>::shift(int val) {
  m_signal = m_signal.shift(val);
}

template <class T> void csignal<T>::cshift(int val) {
...
template <class T> T csignal<T>::max() { return m_signal.max(); }

template <class T> T csignal<T>::min() { return m_signal.min(); }

template <class T> T csignal<T>::sum() { return m_signal.sum(); }

template <class T> void csignal<T>::swap(csignal<T> &signal) {
...
template <class T> uint32_t csignal<T>::azimuthVolume() {
  return m_simpleSizeAzimuth;
}

template <class T> auto csignal<T>::begin() {
  return std::begin(this->m_signal);
}

template <class T> auto csignal<T>::begin() const {
  return std::begin(this->m_signal);
}

template <class T> auto csignal<T>::end() { return std::end(this->m_signal); }

template <class T> auto csignal<T>::end() const {
  return std::end(this->m_signal);
}

template <class T> template <class U> csignal<T>::operator csignal<U>() {
...
template <> csignal<double>::operator csignal<unsigned char>();
...
//глобальные функции
template <class T> csignal<T> operator+(const csignal<T> &lhs, const T &val) {
  csignal<T> temp(lhs);
  temp += val;
  return temp;
}

template <class T> csignal<T> operator+(const T &val, const csignal<T> &rhs) {
  csignal<T> temp(rhs);
  temp += val;
  return temp;
}
...
template <class T> csignal<T> operator/(const csignal<T> &lhs, const T &rhs) {
  csignal<T> temp(lhs);
  temp /= rhs;
  return temp;
}
...
csignal<T> operator/(const csignal<T> &lhs, const csignal<T> &rhs) {
  if (lhs.size() == rhs.size()) {
    ...
    temp.setServiceParameters(lhs.azimuth(), lhs.frameNumber(),
                              lhs.board_number());
    return temp;
  } else
    throw csignal<double>::CsignalLengthMismatch();
...
template <class T> csignal<T> operator*(const T &lhs, const csignal<T> &rhs) {
...
template <class T> csignal<T> Square(const csignal<T> &sig) {
  return sig * sig;
}
...
template <class T>
csignal<T> CorelationForFilter(const csignal<T> &sig,
                               const csignal<T> &filter) {
...
csignal<T> Convolution(const csignal<T> &first, const csignal<T> &second) {
  csignal<T> temp(first.size() + second.size() - 1);
  temp.setServiceParameters(first.azimuth(), first.range(), first.frameNumber(),
                            first.board_number());
  for (size_t i = 0; i < first.size(); i++)
    for (size_t j = 0; j < second.size(); j++)
...
csignal<U> &operator>>(Tstreem &in, csignal<U> &signal) {
  ...
  in.read(reinterpret_cast<char *>(&signal.m_frameNumber),
          sizeof signal.m_frameNumber);
  in.read(reinterpret_cast<char *>(&csignal<U>::m_simpleSizeAzimuth),
          sizeof csignal<U>::m_simpleSizeAzimuth);
  for (auto &sig : signal) {
    in.read(reinterpret_cast<char *>(&sig), sizeof(U));
  }
  return signal;
}
...
void operator<<(Tstreem &out, const csignal<U> &signal) {
  out.write(reinterpret_cast<char const *>(&signal.m_azimuth),
            sizeof signal.m_azimuth);
  out.write(reinterpret_cast<char const *>(&signal.m_range),
            sizeof signal.m_range);
  out.write(reinterpret_cast<char const *>(&signal.m_frameNumber),
            sizeof signal.m_frameNumber);
  out.write(reinterpret_cast<char const *>(&csignal<U>::m_simpleSizeAzimuth),
            sizeof csignal<U>::m_simpleSizeAzimuth);
  for (auto &sig : signal) {
    out.write(reinterpret_cast<char const *>(&sig), sizeof(U));
  }
}
...
csignal<double> AbsComplex(const csignal<double> &reSig,
                           const csignal<double> &imSig);

csignal<double> ArgComplex(const csignal<double> &reSig,
                           const csignal<double> &imSig);

csignal<double> ComplexIm(const csignal<double> &absSig,
                          const csignal<double> &argSig);

csignal<double> ComplexRe(const csignal<double> &absSig,
                          const csignal<double> &argSig);
...
void Complex(const csignal<double> &absSig, const csignal<double> &argSig,
             csignal<std::complex<double>> &result);
...
template <class T> class MovingAverageComplex {
