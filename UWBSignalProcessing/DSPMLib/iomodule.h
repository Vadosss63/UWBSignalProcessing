...
template <typename BufferTypeIn, typename BufferTypeOut>
IOModule<BufferTypeIn, BufferTypeOut>::IOModule(uint8_t inputs,
                                                uint8_t outputs) {
...
template <typename BufferTypeIn, typename BufferTypeOut>
InputPort<BufferTypeIn> *
IOModule<BufferTypeIn, BufferTypeOut>::GetInput(uint8_t port) {
  return m_inputPorts.at(port);
}

template <typename BufferTypeIn, typename BufferTypeOut>
OutputPort<BufferTypeOut> *
IOModule<BufferTypeIn, BufferTypeOut>::GetOutput(uint8_t port) {
...
template <typename BufferTypeIn, typename BufferTypeOut>
void IOModule<BufferTypeIn, BufferTypeOut>::SetInputPortsNumber(
    uint8_t number) {
...
template <typename BufferTypeIn, typename BufferTypeOut>
void IOModule<BufferTypeIn, BufferTypeOut>::SetOutputPortsNumber(
    uint8_t number) {
...
void IOModule<BufferTypeIn, BufferTypeOut>::QueueOperation() {
  if (std::all_of(
          m_inputPorts.begin(), m_inputPorts.end(),
...
void IOModule<BufferTypeIn, BufferTypeOut>::StopEvent() {
  std::for_each(m_inputPorts.begin(), m_inputPorts.end(),
                [](InputPort<BufferTypeIn> *ip) { ip->ClearDataQueue(); });
}

template <typename BufferTypeIn, typename BufferTypeOut>
void IOModule<BufferTypeIn, BufferTypeOut>::PushToOutput(
    const BufferTypeOut &element, uint8_t outputNo) {
  m_outputPorts.at(outputNo)->PushToOutput(element);
}

template <typename BufferTypeIn, typename BufferTypeOut>
void IOModule<BufferTypeIn, BufferTypeOut>::PushToOutput(
    BufferTypeOut &&element, uint8_t outputNo) {
  m_outputPorts.at(outputNo)->PushToOutput(std::move(element));
}

template <typename BufferTypeIn, typename BufferTypeOut>
bool IOModule<BufferTypeIn, BufferTypeOut>::PopFromInput(BufferTypeIn &element,
                                                         uint8_t inputNo) {
  return m_inputPorts.at(inputNo)->PopFromInput(element);
}

template <typename BufferTypeIn, typename BufferTypeOut>
void IOModule<BufferTypeIn, BufferTypeOut>::ReturnInputBuffer(
    BufferTypeIn &&element, uint8_t inputNo) {
