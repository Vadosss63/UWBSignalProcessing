...
TargetItem::TargetItem(const Target &target)
  ...
  CreateItem([](double x) { return SectorGrid::Val_XToPixelLeftPanel(x); },
             [](double y) { return SectorGrid::Val_YToPixelLeftPanel(y); });
}

void TargetItem::ShowZoomItem() {
  if (m_zoomItem) {
    if (!SectorGrid::ContenesZoom(
            m_zoomItem->m_target.GetY(m_target.GetFont())))
...
void TargetItem::SetMaxAMP(int maxAMP) { m_maxAMP = maxAMP; }

void TargetItem::SetMaxAMPdymamic(int max) { m_maxAMPdymamic = max; }

int TargetItem::GetMaxAMP() { return m_maxAMP; }

int TargetItem::GetMaxAMPdymamic() { return m_maxAMPdymamic; }

double TargetItem::GetCurrentSpeed() {
  return m_target.GetToSpeedKMH(m_target.m_currentSpeed);
}

void TargetItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
                       QWidget *) {
  painter->save();
...
TargetItem *TargetItem::GetItemZoom() {
  if (!m_zoomItem) {
    ...
    m_zoomItem->CreateItem(
        [](double x) { return SectorGrid::Val_XToPixel(x); },
        [](double y) { return SectorGrid::Val_YToPixel(y); });
  }
  return m_zoomItem;
}

QPen TargetItem::GetPen() const { return m_pen; }

void TargetItem::SetPen(const QPen &pen) { m_pen = pen; }

QRectF TargetItem::boundingRect() const { return m_boundingRect; }

void TargetItem::CreateItem(const std::function<double(double)> &getX,
  ...
  for (Target::Point &point : m_target.GetPoints()) {
    QPointF p = QPointF(getX(m_target.GetX(point)), getY(m_target.GetY(point)));
