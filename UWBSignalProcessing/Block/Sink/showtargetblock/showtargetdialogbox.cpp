...
QList<std::pair<QString, QVariant>> ShowTargetDialogBox::GetSetting()
{
    ...
    listSetting.append(std::make_pair("maxAMPdymamic",  TargetItem::GetMaxAMPdymamic()));
    return listSetting;
}

bool ShowTargetDialogBox::SetSetting(QList<std::pair<QString, QVariant>> listSetting)
{
...
void ShowTargetDialogBox::resizeEvent(QResizeEvent *)
{
    ...
    if(m_grid)
        m_view->fitInView(m_grid->boundingRect().marginsAdded(qMarg), Qt::KeepAspectRatio);
    else
        m_view->fitInView(m_scene->sceneRect().marginsAdded(qMarg), Qt::KeepAspectRatio);
...
//        rectTarget.moveCenter(QPointF(SectorGrid::Val_XToPixel(x), SectorGrid::Val_YToPixel(y)));
//        QLabel* label = new QLabel();
//        QMovie* movie = new QMovie(":/man.gif");
...
{
    DialogSettingTarget dialog(TargetItem::GetMaxAMP(), TargetItem::GetMaxAMPdymamic());
    if(dialog.exec())
    {
...
void ShowTargetDialogBox::SendCommand()
{
...
void ScanView::ZoomIn()
{
    scale(1.5, 1.5);
}

void ScanView::ZoomOriginal()
{
    resetTransform();
}

void ScanView::ZoomOut()
{
    scale(0.75, 0.75);
}
...
DialogSettingTarget::DialogSettingTarget(int currentVal, int currentValDynamic)
{
...
int DialogSettingTarget::GetMaxVal()
{
    return m_maxVal->value();
}

int DialogSettingTarget::GetMaxValDynamic()
{
    return m_maxValDynamic->value();
}
