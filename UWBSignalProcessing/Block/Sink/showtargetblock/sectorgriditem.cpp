...
QRectF SectorGrid::m_leftPanel =
    QRectF(-1.5 * SectorGrid::m_widthLeftPanel, 0, SectorGrid::m_widthLeftPanel,
           SectorGrid::m_height);

SectorGrid::SectorGrid(QGraphicsItem *parent) : QGraphicsPathItem(parent) {
...
void SectorGrid::ShowCurrentSpeed(double speed) {
  if (!m_valueSpeed) {
    ...
    m_valueSpeed->setPos(GetRect().topRight().x(),
                         GetRect().topRight().y() + 100);
    m_valueSpeed->setDefaultTextColor(GetColorGrid());
  }
  m_valueSpeed->setPlainText(
      QString("v = %1 км/ч").arg(QString::number(speed, 2, 1)));
}
...
const QColor &SectorGrid::GetColorGrid() const { return m_colorGrid; }

qreal SectorGrid::MaxVal_Y() { return m_maxVal_YZoom; }

qreal SectorGrid::MinVal_Y() { return m_minVal_YZoom; }

qreal SectorGrid::MaxVal_X() { return m_maxVal_XZoom; }

QRectF SectorGrid::GetRect() const { return m_rectGrid; }

void SectorGrid::SetRect(const QRectF &rect) { m_rectGrid = rect; }
...
void SectorGrid::CreateAxisX(QPainter *painter, QPainterPath &path) {
  ...
  for (qreal i = 0; i <= numTicks; i++) {
    ...
    QString strText =
        QString::number(-maxVal_X + ((2 * maxVal_X) / numTicks) * i, 2, 0);
...
void SectorGrid::CreateAxisY(QPainter *painter, QPainterPath &path) {
  ...
  for (qreal i = 0; i <= m_numTicks; i++) {
    ...
    QString strText =
        QString::number(minRange + (delta / m_numTicks) * i, 2, 0);
...
void SectorGrid::paint(QPainter *painter, const QStyleOptionGraphicsItem *,
                       QWidget *) {
...
QPainterPath SectorGrid::GetZoneLook(const double constVal) const {
  ...
  path.lineTo(Val_XToPixel(leftBottomPoint.x()),
              Val_YToPixel(leftBottomPoint.y()));
  path.moveTo(Val_XToPixel(rightBottomPoint.x()),
              Val_YToPixel(rightBottomPoint.y()));
...
void SectorGrid::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  if (m_rectGrid.contains(event->pos())) {
    ...
    m_value->setPlainText(QString("x = %2 м\ny = %1 м")
                              .arg(QString::number(range, 2, 1))
                              .arg(QString::number(velocity, 2, 1)));
    return;
  }
