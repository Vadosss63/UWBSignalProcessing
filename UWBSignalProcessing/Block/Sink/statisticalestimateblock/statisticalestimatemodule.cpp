...
StatisticaleStimateModule::StatisticaleStimateModule() :
    SinkModule(1){}
...
void StatisticaleStimateModule::Operate()
{
    ...
    if(m_showStatisticaleStimate)
        m_showStatisticaleStimate->ShowStimate(mean, meanValue, variance, osv, osv / meanValue);
...
double StatisticaleStimateModule::Variance(const csignal<double>& signal, double mean)
{
    double variance = 0;
    std::for_each(signal.begin() + m_startIndex, signal.begin() + m_stopIndex + 1,
                  [&variance, mean](double val)
    {
...
void StatisticaleStimateModule::SetShowStatisticaleStimate(ShowStatisticaleStimate* showStatisticaleStimate)
{
