...
const QColor& SectorGridBaseItem::GetColorGrid() const
{
    return m_colorGrid;
}

float SectorGridBaseItem::MaxRange() const
{
    return m_maxRange;
}
...
float SectorGridBaseItem::StopBearing() const
{
    return m_stopBearing;
}

float SectorGridBaseItem::StartBearing() const
{
    return m_startBearing;
}
...
void SectorGridBaseItem::SetPathMousePressEvent(const QPainterPath &pathMousePressEvent)
{
...
float SectorGridBaseItem::Resolution() const
{
    return m_resolution;
}
...
{
    SetRect(QRectF(- GetSizeRect(),- GetSizeRect(), 2 * GetSizeRect(), GetSizeRect()));
}

// Переопределенный метод рисования сетки
void SectorRectGridItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    painter->setRenderHint(QPainter::Antialiasing);
    QPainterPath path;
    ...
    else
    {
        ...
        stopAng = startAng + ((StopBearing() - StartBearing()) / stepBering) * stepAngel;
        qreal end ;
    ...
    for (qreal i = -GetSizeRect(); i <= 0; i += GetSizeRect() / 5)
    {
        ...
        QString text(QString::number(m_maxRange - 2048 * m_resolution - (2 * (i) * m_resolution) , 2, 0));
...
SectorRoundGridItem::SectorRoundGridItem(QGraphicsItem *parent)
    : SectorGridBaseItem(parent)
{
    SetRect(QRectF(- GetSizeRect(), - GetSizeRect(), 2 * GetSizeRect(), 2 * GetSizeRect()));
}

// Переопределенный метод рисования сетки
void SectorRoundGridItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    painter->setRenderHint(QPainter::Antialiasing);
    QPainterPath path;
    ...
    for (qreal i = 0; i < 360; i +=15)
    {
        ...
        QPointF pointStart = QPointF(GetRectPositionSectors().center().rx() + xS, GetRectPositionSectors().center().ry() + yS);
        path.moveTo(pointStart);
    ...
    for (int i = 0; i < 6; i++)
    {
        QRectF Rec(GetRectPositionSectors().x() + 150 * i, GetRectPositionSectors().y() + 150 * i, GetRectPositionSectors().width() - 300 * i, GetRectPositionSectors().height() - 300 * i);
        ...
        QString textRan(QString::number((static_cast<double>(m_resolution)*(Rec.height()))/1000, 3, countNumb));
...
void SectorRoundGridItem::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    qreal range = std::sqrt(std::pow(event->pos().x(), 2) + std::pow(event->pos().y(),2));
    //Рисование дуг сетки
    QRectF Rec(GetRectPositionSectors().topLeft().x() + (GetSizeRect() - range), GetRectPositionSectors().topLeft().x() + (GetSizeRect() - range), 2*range, 2*range);
...
/////////////////////////////////////////////////////////////////////////////////////////////////
SectorImageItem::SectorImageItem(qreal startAng, qreal stepAng, QColor color, QGraphicsItem *parent):QGraphicsPathItem(parent),
    m_color(std::move(color)), m_rect(-1024, -1024, 2048, 2048), m_startAng(startAng), m_stepAng(stepAng) {}

void SectorImageItem::SetBrush(bool onBrash)
{
    m_brash = onBrash;
}

void SectorImageItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    if(m_brash)
...
// Переопределенный метод рисования сетки
void SectorArcGridItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget* )
{
...
//        m_rectText.moveCenter(QPoint(0, -(GetSizeRect() + m_sizeTextRect.height() / 2)));
...
void SectorArcGridItem::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    //    qreal angle = atan(event->pos().x() / event->pos().y() ) * 180 / M_PI + 180;
    //    qreal xS = GetSizeRect() * std::sin((2 * M_PI * (angle)) / 360);
    //    qreal yS = GetSizeRect() * std::cos((2 * M_PI * ( angle)) / 360);
    //    QPointF pointStart = QPointF(GetRectPositionSectors().center().rx() + xS, GetRectPositionSectors().center().ry() + yS);
    //    PathMousePressEvent().moveTo(pointStart);
    //    PathMousePressEvent().lineTo(GetRectPositionSectors().center());
    //    qreal range = sqrt(event->pos().x() * event->pos().x() + event->pos().y() * event->pos().y());
    //    //Рисование дуг сетки
    //    QRectF Rec(GetRectPositionSectors().topLeft().x() +(GetSizeRect() - range) , GetRectPositionSectors().topLeft().x() + (GetSizeRect() - range), 2*range, 2*range);
    //    PathMousePressEvent().arcMoveTo(Rec, 45);
...
void SectorArcGridItem::CreateGrid()
{
    ...
    // облать отрисовки
    m_boundingRect = QRectF(-sizeRect / std::sqrt(2),
                            -sizeRect, 2 * (sizeRect/std::sqrt(2)),
                            sizeRect);
...
void SectorArcGridItem::SetVisible(bool isVisible)
{
    m_isVisible = isVisible;
}

QRectF SectorArcGridItem::boundingRect() const
{
