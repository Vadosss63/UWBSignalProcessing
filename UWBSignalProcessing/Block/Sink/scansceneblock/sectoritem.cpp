...
SectorBaseItem::SectorBaseItem(qreal startAnglePos, qreal bering, qreal stepAngle, QGraphicsItem *parent):
    QGraphicsItem(parent),
    m_startAnglePos(startAnglePos), m_stepAngle(stepAngle), m_activeWhiteSector(false),
    m_bering(bering), m_pen(QColor(Qt::transparent))
{
    m_pen.setStyle(Qt::DotLine);
    m_pen.setCapStyle(Qt::RoundCap);
}

SectorRect::SectorRect(qreal startAnglePos, float bering, qreal stepAngle, QGraphicsItem *parent): SectorBaseItem(startAnglePos, bering, stepAngle, parent)
{
    CreatItem();
}
...
void SectorRect::CreatItem()
{
    ...
    QLinearGradient* gradientImage = new QLinearGradient(sRect.bottomLeft(), sRect.topLeft());
...
qreal SectorBaseItem::StepAngle() const
{
    return m_stepAngle;
}
...
qreal SectorBaseItem::StartAnglePos() const
{
    return m_startAnglePos;
}
...
void SectorBaseItem::SetGradientWhiteSector(const QLinearGradient &gradientWhiteSector)
{
...
size_t SectorBaseItem::CountPoint() const
{
    return m_countPoint;
}
...
qreal SectorBaseItem::Bering() const
{
    return m_bering;
}

void SectorBaseItem::SetBering(qreal bering)
{
    m_bering = bering;
}

qreal SectorBaseItem::SizeRect() const
{
    return m_sizeRect;
}
...
SectorArc::SectorArc(qreal startAnglePos, qreal bering, qreal stepAngle, QGraphicsItem *parent):
...
//    qreal bering = (std::arg(std::complex<qreal>(static_cast<qreal>(-y), static_cast<qreal>(x)))) * 180 / (M_PI);
//    if(x < 0)
//        bering += 360;
...
void SectorArc::CreatItem()
{
    ...
    path.arcTo(sRect, 90 - StartAnglePos(), -StepAngle() - shiftArc);// построение по часовой стрелки
    path.lineTo(sRect.center());
    SetPath(path);

    qreal x = SizeRect() * std::sin((2.0 * 3.14 * (180.0 - StartAnglePos()))/360);
    qreal y = SizeRect() * std::cos((2.0 * 3.14 * (180.0 - StartAnglePos()))/360);

    QPointF pointStop = QPointF(x, y);
    QLinearGradient* gradientImage = new QLinearGradient(sRect.center(), sRect.center() + pointStop);
    gradientImage->setColorAt(0, Qt::black);
    gradientImage->setColorAt(1, Qt::black);
    SetGradientImage(gradientImage);


    qreal xS = SizeRect() * std::sin((2 * 3.14 * (180.0 - StartAnglePos() - StepAngle()))/360);
    qreal yS = SizeRect() * std::cos((2 * 3.14 * (180.0 - StartAnglePos() - StepAngle()))/360);
    QPointF pointStart = QPointF(xS, yS);

    QLinearGradient gradientWhiteSector(sRect.center() + pointStart, sRect.center() + pointStop);
