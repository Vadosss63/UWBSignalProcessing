...
//////////////////
ScanSceneBlock::ScanSceneBlock() : AbstractScanBlock("ScanScene") {
  setToolTip("Выводит радиолокационное изображение");
}

AbstractBlock *ScanSceneBlock::Clone() const { return new ScanSceneBlock(); }

AbstractModule *ScanSceneBlock::GetModule() const { return m_module.get(); }

QString ScanSceneBlock::GetType() const { return "ScanSceneBlock"; }

void ScanSceneBlock::Change() {
  if (m_moduleVrli && m_dialog) {
    m_moduleVrli->SetCoeffRange(
        static_cast<float>(m_dialog->GetCoefStreatch()) / 2);
    m_dialog->ClearVRLI();
  }
}
...
AbstractBlock *PluginBlock::LoudBlock() const { return new ScanSceneBlock; }

void ScanSceneBlock::RegistrOperationManager(
