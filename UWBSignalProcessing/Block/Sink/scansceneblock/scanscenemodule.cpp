...
void ScanBufferSceneModule::UpdateScan(uint32_t azimuth) {
  ...
  auto res_time = std::chrono::duration_cast<std::chrono::milliseconds>(
      end_time - m_lastTimeUpdate);
...
void ScanBufferSceneModule::Transform(csignal<double> &newSignal) {
  for (size_t i = m_startIndexPlot, index = 0; i < m_stopIndexPolot + 1;
       ++i, ++index)
    SetValColor(static_cast<int>(index), newSignal[i]);

  m_dataMatrix[newSignal.azimuth()]->GetGradientImage()->setStops(
      m_gradientPoints);
}
...
void ScanBufferSceneModule::SetUpdateScan(bool updateScan,
                                          int updateStep) noexcept {
...
void ScanBufferSceneModule::SetValue(int verticalShift, int gain,
                                     uint8_t coefStreatch) {
  std::lock_guard<std::mutex> lock(operateLock);
