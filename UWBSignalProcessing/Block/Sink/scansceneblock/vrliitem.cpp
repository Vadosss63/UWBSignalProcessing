...
void VRLIItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
...
qreal VRLIItem::GetV() const
{
    return m_v;
}

void VRLIItem::SetV(qreal v)
{
    m_v = v;
}

qreal VRLIItem::GetAngle() const
{
    return m_angle;
}

void VRLIItem::SetAngle(const qreal &startAnglePos)
{
    m_angle = startAnglePos;
}
...
QPen VRLIItem::GetPen() const
{
    return m_pen;
}

void VRLIItem::SetPen(const QPen &pen)
{
    m_pen = pen;
}

qreal VRLIItem::GetRange() const
{
    return m_range;
}

void VRLIItem::SetRange(qreal range)
{
    m_range = range;
}


QRectF VRLIItem::boundingRect() const
{
...
void VRLIItem::SetPos(qreal range, qreal angle)
{
    ...
    QGraphicsItem::setPos(x - m_boundingRect.width() / 2, y - m_boundingRect.height()/2);
}
