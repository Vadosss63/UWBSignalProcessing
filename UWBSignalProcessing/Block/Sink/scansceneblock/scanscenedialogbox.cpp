...
ScanSceneDialogBox::ScanSceneDialogBox(ScanBufferSceneModule *mParent, QWidget *wparent) :
    ...
    m_module(mParent), m_grid(nullptr)
{
    ...
    m_scene = new QGraphicsScene( -m_sizeRect / std::sqrt(2), -  m_sizeRect, 2 * (m_sizeRect/std::sqrt(2) ), m_sizeRect, this);
    ...
    connect(m_panelScan->GetUi()->pushButton, SIGNAL(clicked()),this, SLOT(SendCommandAbstractModule()) );
    connect(m_panelScan->GetUi()->zoomIn, SIGNAL(clicked()), this, SLOT(ZoomIn()));
    connect(m_panelScan->GetUi()->zoomOut, SIGNAL(clicked()), this, SLOT(ZoomOut()));
    connect(m_panelScan->GetUi()->isLineOnScan, SIGNAL(clicked(bool)), this, SLOT(SetLineVisible(bool)));
    connect(m_panelScan->GetUi()->isColorScan, SIGNAL(clicked(bool)), this, SLOT(SetViewColor(bool)));
    connect(m_panelScan->GetUi()->isScanMode, SIGNAL(clicked(bool)), this, SLOT(SetScanMode(bool)));
...
ScanSceneDialogBox::~ScanSceneDialogBox()
{
    WriteSettingColor();
}
...
QList<std::pair<QString, QVariant>> ScanSceneDialogBox::GetSetting()
{
    ...
    listSetting.push_back(std::make_pair(QString("CoefStreatch"), GetCoefStreatch()));
    listSetting.push_back(std::make_pair(QString("VerticalShift"), GetVerticalShift()));
    ...
    listSetting.push_back(std::make_pair(QString("IsColorView"), GetIsColorView()));
    listSetting.push_back(std::make_pair(QString("IsScanLine"), GetIsScanLine()));
    listSetting.push_back(std::make_pair(QString("IsGridOnScan"), GetIsGridOnScan()));
    return listSetting;
}

bool ScanSceneDialogBox::SetSetting(QList<std::pair<QString, QVariant>> listSetting)
{
...
void ScanSceneDialogBox::ZoomIn()
{
    m_view->scale(1.1, 1.1);
}

void ScanSceneDialogBox::ZoomOut()
{
    m_view->scale(1 / 1.1, 1 / 1.1);
}

void ScanSceneDialogBox::SetRange(uint32_t val)
{
    m_panelScan->SetRange(val);
}
...
{
    m_module->SetValue(m_panelScan->GetVerticalShift(),
                       m_panelScan->GetGain(),
    ...
    if(m_grid)
    {
        float resolution = 2.f * m_currentResolution / m_panelScan->GetCoefStreatch();
...
{
    m_module->SetUpdateScan(m_panelScan->GetUpdateScan(), m_panelScan->GetUpdateStep());
}

void ScanSceneDialogBox::UpdataScene()
{
    m_scene->update();
}
...
{
    QColor color = QColorDialog::getColor(m_grid->GetColorGrid(), this, "Цвет сетки");
    if (color.isValid())
    {
...
{
    QColor color = QColorDialog::getColor(VRLIItem::GetColorItem(), this, "Цвет целей");
    if (color.isValid())
    {
...
void ScanSceneDialogBox::CreateGrid()
{
    ...
    connect(m_panelScan->GetUi()->isGridOnScan, SIGNAL(clicked(bool)), this, SLOT(SetGrid(bool)));
    connect(m_grid, SIGNAL(SelectPoint(qreal, qreal)), this , SLOT(DumpEvent(qreal, qreal)));
...
void ScanSceneDialogBox::CreateSector(int simpleSizeAzimuth)
{
    ...
    for(qreal bering = 0, i = -45; i < 45; i += stepAngel, bering++)
    {
        ...
        //        connect(sector, SIGNAL(PressEvent(qreal, qreal)), this , SLOT(DumpEvent(qreal, qreal)));
        AddSector(sector);
    }
    m_scene->update();
}

void ScanSceneDialogBox::SendCommand()
{
...
int ScanSceneDialogBox::GetGain() const
{
    return m_panelScan->GetGain();
}
...
void ScanSceneDialogBox::SetGain(int gain)
{
    m_panelScan->SetGain(gain);
}
...
void ScanSceneDialogBox::CreateStyle()
{
    ...
    m_view->setStyleSheet("*{border: 2px solid white; border-radius: 10px; border: 1px solid white; border-radius: 5px;}"

                          );

    m_view->verticalScrollBar()->setStyleSheet("QAbstractScrollArea::QScrollBar{"
                                               " background-color: rgb(150, 150, 150); "
                                               "}"

                                               "QAbstractScrollArea::QScrollBar::handle:vertical, QScrollBar::handle:horisontal{"
                                               "background-color: rgb(100, 100, 100); "
                                               "margin: 10px;"
                                               " padding 10px;"
                                               "}");

    m_view->horizontalScrollBar()->setStyleSheet("QAbstractScrollArea::QScrollBar{"
                                                 " background-color: rgb(150, 150, 150); "
                                                 "}"
    ...
    m_panelScan->setStyleSheet("*{color: white;background-color: black;}"
                               "QGroupBox {"
                               "margin: 5 ; border: 1px solid white; border-radius: 5px; padding: 10px 4px 4px 0px; "
                               "}"
                               "QGroupBox::title {"
                               "margin: 0 ; subcontrol-position: center top;"
                               "}"
                               "QPushButton {"
                               "min-width: 16px; background: qlineargradient(x1:0, y1:1, x2:0, y2:0, stop:1 rgb(133, 133, 135), stop:0.4 rgb(31, 31, 33,));"
                               "color: white; border: 1px solid white; border-radius: 5px;"
                               "}"
                               "QPushButton:hover {"
                               "background: qlineargradient(x1:0, y1:1, x2:0, y2:0, stop:1 rgb(133, 133, 135), stop:0.4 rgb(31, 31, 33), stop:0.2 rgb(0, 0, 150,) );"
                               "}"
                               "QPushButton:pressed {background: qlineargradient(x1:0, y1:1, x2:0, y2:0, stop:0 rgba(1, 1, 5, 80), stop:0.6 rgba(18, 18, 212, 80), stop:0.5 rgba(142, 142, 245, 80) );"
                               ...
                               "}"
                               "QSpinBox::up-button {"
                               "background: qlineargradient(x1:0, y1:1, x2:0, y2:0, stop:1 rgb(133, 133, 135), stop:0.4 rgb(31, 31, 33,));"
                               "border: 1px solid  white; border-top-right-radius: 10px;"
                               "border-top-left-radius: 3px; subcontrol-position: top right; subcontrol-origin: padding;"
                               "}"
                               "QSpinBox::up-button:pressed{"
                               "background: qlineargradient(x1:0, y1:1, x2:0, y2:0, stop:0 rgba(1, 1, 5, 80), stop:0.6 rgba(18, 18, 212, 80), stop:0.5 rgba(142, 142, 245, 80) );"
                               "border: 1px solid rgb(18, 18, 212);"
                               "    }"
                               ...
                               "background: qlineargradient(x1:0, y1:1, x2:0, y2:0, stop:1 rgb(133, 133, 135), stop:0.4 rgb(31, 31, 33,));"
                               "border: 1px solid  white; border-bottom-right-radius: 10px; border-bottom-left-radius: 3px; subcontrol-position: bottom right; subcontrol-origin: padding;"
                               "    }"
