...
SectorBaseItem::SectorBaseItem(qreal start, qreal count, QGraphicsItem *parent)
    : QGraphicsItem(parent), m_start(start), m_pen(QColor(Qt::transparent)) {
  m_step = m_height / count;
  m_pen.setStyle(Qt::DotLine);
  m_pen.setCapStyle(Qt::RoundCap);
}

SectorRect::SectorRect(qreal start, qreal count, QGraphicsItem *parent)
    : SectorBaseItem(start, count, parent) {
  CreatItem();
}
...
void SectorRect::CreatItem() {
  ...
  QLinearGradient *gradientImage =
      new QLinearGradient(rect.bottomLeft(), rect.bottomRight());
...
qreal SectorBaseItem::Step() const { return m_step; }

qreal SectorBaseItem::Start() const { return m_start; }

void SectorBaseItem::SetStart(qreal startAnglePos) { m_start = startAnglePos; }
...
qreal SectorBaseItem::Width() const { return m_width; }

qreal SectorBaseItem::Height() const { return m_height; }

void SectorBaseItem::SetHeight(qreal height) { m_height = height; }

void SectorBaseItem::SetWidth(qreal width) { m_width = width; }
