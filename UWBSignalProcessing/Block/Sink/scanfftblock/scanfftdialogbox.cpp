...
bool ScanfftDialogBox::SetSetting(QList<std::pair<QString, QVariant>> listSetting)
{
...
void ScanfftDialogBox::UpdataScene()
{
    m_scene->update();
}
...
int ScanfftDialogBox::GetLinePloter() const
{
    return m_linePloter;
}

bool ScanfftDialogBox::GetIsLog() const
{
    return m_isLog;
}
...
void ScanfftDialogBox::SendCommand()
{
    SendCommandAbstractModule();
}

void ScanfftDialogBox::Attach(ChangeEvent* observer)
{
    m_observer = observer;
}
...
CountRangeDialog::CountRangeDialog(QWidget* parent): QDialog (parent, Qt::WindowTitleHint| Qt::WindowSystemMenuHint)
{
    CreateDialog();
}

int CountRangeDialog::GetCount()
{
    return m_countRange->value();
}
...
qreal CountRangeDialog::MaxVelocity()
{
    return m_F->value() * 0.1 / 4;
}

void CountRangeDialog::SetCount(int count)
{
    m_countRange->setValue(count);
}
...
void CountRangeDialog::SetFrec(int frec)
{
    m_F->setValue(frec);
}

int CountRangeDialog::GetResolution()
{
    return m_resolution->value();
}

int CountRangeDialog::GetFrec()
{
    return m_F->value();
}
...
void SettingDialog::SetIsLog(bool isLog)
{
    m_isLog->setChecked(isLog);
}

bool SettingDialog::IsLog()
{
    return m_isLog->isChecked();
}
...
int SettingDialog::GetLinePloter()
{
    return m_linePloter->value();
}
