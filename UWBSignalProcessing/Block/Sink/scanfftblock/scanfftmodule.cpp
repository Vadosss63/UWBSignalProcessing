...
void ScanfftModule::Transform(std::vector<std::valarray<double> >& newSignal)
{
    if(m_isVertical)/// TODO переделать это костыль
    {
        ...
        for (size_t i = 0; i < newSignal.size(); ++i)
        {
            ...
            m_updateScane->DataMatrix()[i]->GetGradientImage()->setStops(m_gradientPoints);
        }
...
void ScanfftModule::SetValColor(int pos, double val)
{
    if(m_isLog)
    {
        ...
        m_gradientPoints[pos].second = QColor(static_cast<int>(val), static_cast<int>(val), static_cast<int>(val));
...
void ScanfftModule::SetPloterLine(int ploterLine)
{
    m_ploterLine = ploterLine;
}
...
void ScanfftModule::SetIsLog(bool isLog)
{
    m_isLog = isLog;
}
