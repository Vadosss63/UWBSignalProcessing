...
qreal SectorGrid::MaxRange() const { return m_maxRange; }

void SectorGrid::SetMaxRange(qreal maxRange) { m_maxRange = maxRange; }

qreal SectorGrid::GetMinRange() const { return m_minRange; }

void SectorGrid::SetMinRange(qreal minRange) { m_minRange = minRange; }

qreal SectorGrid::GetMaxVelocity() const { return m_maxVelocity; }

void SectorGrid::SetMaxVelocity(qreal velocity) { m_maxVelocity = velocity; }
...
void SectorGrid::CreateAxisX(QPainter *painter, QPainterPath &path) {
  ...
  for (qreal i = 0; i <= numTicks; i++) {
    ...
    QString strText =
        QString::number(-maxV + ((2 * maxV) / numTicks) * i, 2, 0);
