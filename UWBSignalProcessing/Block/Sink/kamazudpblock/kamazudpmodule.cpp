...
KamazUDPModule::KamazUDPModule() :
    SinkModule(1), m_server(nullptr)
{
    m_dataSizeToSend = sizeof (KamazDatagram::HeaderPRLI) + sizeof (KamazDatagram::DataPRLI);
...
void KamazUDPModule::CreateProtocol(csignal<double>& sig)
{
    ...
    for (uint8_t i = 0; i < size; ++i)
    {
        ...
        std::copy(sig.begin() + maxSizeData * i, sig.begin() +
                  (maxSizeData * i + sigData.size()), sigData.begin());

        sigData.setAzimuth(sig.azimuth());


        KamazDatagram::HeaderPRLI* headerPRLI = reinterpret_cast<KamazDatagram::HeaderPRLI*>(m_dataSend);
        ...
        headerPRLI->significantNumberOfDiscrete = static_cast<uint32_t>(sigData.size());// Количество значемых дискретов
        headerPRLI->cyclicMessageNumber = getCyclicNum();
        headerPRLI->bearing = sigData.azimuth(); // угол места
        headerPRLI->rangeOfFirstElement = static_cast<uint32_t>(sigData.size() * i * discret); // Дальность первого элемента массива

        const size_t sheftHeader = sizeof (KamazDatagram::HeaderPRLI);
        KamazDatagram::DataPRLI* dataPRLI = reinterpret_cast<KamazDatagram::DataPRLI*>(&m_dataSend[sheftHeader]);
