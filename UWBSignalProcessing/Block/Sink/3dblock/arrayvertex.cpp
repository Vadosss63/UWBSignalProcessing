...
ArrayData::ArrayData(size_t countPoint)
    : m_countPoint(static_cast<GLsizei>(countPoint)),
      m_massiveVertex(countPoint * 3), m_massiveColor(countPoint * 3, 128) {
  SetXmas();
  SetZmas();
}
...
GLsizei ArrayData::CountPoint() const { return m_countPoint; }
...
GLfloat ArrayData::getMaxX() { return m_maxX; }

void ArrayData::setMaxX(const GLfloat &maxX) { m_maxX = maxX; }

GLfloat ArrayData::getMaxY() { return m_maxY; }

void ArrayData::setMaxY(const GLfloat &maxY) { m_maxY = maxY; }

GLfloat ArrayData::getMaxZ() { return m_maxZ; }

void ArrayData::setMaxZ(const GLfloat &maxZ) { m_maxZ = maxZ; }
