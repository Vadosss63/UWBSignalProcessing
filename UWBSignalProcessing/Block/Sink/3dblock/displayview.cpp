...
DisplayView::~DisplayView() { delete m_drawData; }

void DisplayView::timerEvent(QTimerEvent *) { ReadData(); }

void DisplayView::Start() { m_idTimer = startTimer(20); }

void DisplayView::Pause() { killTimer(m_idTimer); }
...
void DisplayView::OpenFile() {
  QString path =
      QFileDialog::getOpenFileName(this, "Открыть файл", "", "rmp(*.rmp)");
  if (path.isEmpty())
    return;
...
void DisplayView::AddViewOrig(DrawLine *drawLine) {
  ...
  connect(m_drawDataOriginal, SIGNAL(MaxShift(int)), this,
          SLOT(SetMaximumShift(int)));
}
...
void DisplayView::CreateButtom() {
  ...
  connect(m_sliderNavigate, SIGNAL(valueChanged(int)), this,
          SLOT(SetChangeShift(int)));
...
void DisplayView::RenderNewLine(const std::vector<quint8> &data,
                                bool upadeNow) {
  m_drawData->makeCurrent();
  m_drawData->AddData(data, upadeNow);
}

void DisplayView::RenderNewLine(const std::vector<QColor> &data,
                                bool upadeNow) {
  m_drawData->makeCurrent();
  m_drawData->AddData(data, upadeNow);
}

void DisplayView::RenderNewLineOrig(const std::vector<quint8> &data,
                                    bool upadeNow) {
  m_drawDataOriginal->makeCurrent();
  m_drawDataOriginal->AddData(data, upadeNow);
}

void DisplayView::RenderNewLineOrig(const std::vector<QColor> &data,
                                    bool upadeNow) {
  m_drawDataOriginal->makeCurrent();
  m_drawDataOriginal->AddData(data, upadeNow);
}
