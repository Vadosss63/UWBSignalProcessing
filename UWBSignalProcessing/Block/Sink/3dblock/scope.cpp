...
Scope::Scope(int countChanels, QSize size, QWidget *pwgt)
    : QGLWidget(pwgt), m_sizePaintRect(size) {
...
Scope::~Scope() { glDeleteLists(m_grid, 1); }
...
QSize Scope::SizePaintRect() const { return m_sizePaintRect; }
...
void Scope::SetBackgraundColor(const QColor &color) { qglClearColor(color); }
...
void Scope::UpateLines() { updateGL(); }
...
void Scope::CreateLine(int idLine) {
  ...
  glDrawArrays(GL_LINE_STRIP, 0,
               static_cast<GLsizei>(m_dataLine[idLine].countPoint()));
