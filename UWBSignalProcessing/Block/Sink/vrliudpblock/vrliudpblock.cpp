...
VRLIUDPBlock::VRLIUDPBlock():
    SinkBlock("ВРЛИ UDP"), m_module(nullptr)
{
    setToolTip("Выполняет передачу данных по протоколу UDP");
}

AbstractBlock* VRLIUDPBlock::Clone() const
{
    return new VRLIUDPBlock();
}

AbstractModule* VRLIUDPBlock::GetModule() const
{
    return m_module.get();
}

QString VRLIUDPBlock::GetType() const
{
    return "VRLIUDPBlock";
}

void VRLIUDPBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void VRLIUDPBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
