...
WriteDialogBox::WriteDialogBox() { CreateWidget(); }
...
QList<std::pair<QString, QVariant>> WriteDialogBox::GetSetting() {
  ...
  listSetting.append(
      std::make_pair("m_typeSignal_CB", m_typeSignal_CB->currentIndex()));
  return listSetting;
}

bool WriteDialogBox::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
...
void WriteDialogBox::OpenFile() {
  QString path =
      QFileDialog::getSaveFileName(this, "Сохранение файла", "", "bin(*.bin)");
  TrimmedPath(path);
...
void WriteDialogBox::CreateWidget() {
  ...
  m_typeSignal_CB->addItem("uint_16",
                           static_cast<int>(TypeSignal::uint_16_Type));
