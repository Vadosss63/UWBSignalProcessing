...
AbstractBlock* WriteBlock::Clone() const
{
    return new WriteBlock();
}

AbstractModule* WriteBlock::GetModule() const
{
    return m_module.get();
}

QString WriteBlock::GetType() const
{
    return "WriteBlock";
}
...
void WriteBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void WriteBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
