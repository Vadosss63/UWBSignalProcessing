...
void WriteModule::AddServiceParametersInBin() {
  ...
  if (wreadFile) {
    ...
    wreadFile.write(reinterpret_cast<char const *>(&sizeofType),
                    sizeof sizeofType);
    unsigned short doubleHash = typeid(double).hash_code();
    wreadFile.write(reinterpret_cast<char const *>(&doubleHash),
                    sizeof(doubleHash));
    wreadFile.close();
...
WriteModule::~WriteModule() { m_writeFile.close(); }
...
void WriteModule::SetTypeWrite(TypeSignal type) { m_typeWriteSignal = type; }
