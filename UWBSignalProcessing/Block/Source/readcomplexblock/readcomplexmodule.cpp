...
ReadComplexModule::~ReadComplexModule() { CloseStream(); }

void ReadComplexModule::SetPathFile(const std::string &pathFileSigI,
                                    const std::string &pathFileSigQ) {
  CloseStream();
  m_pathFileSigI = pathFileSigI;
  m_pathFileSigQ = pathFileSigQ;
  m_flagReadFile =
      ReadServiceParametersInBinSigI() && ReadServiceParametersInBinSigQ();
}
...
bool ReadComplexModule::ReadServiceParametersInBinSigI() {
  m_readFileSigI.open(m_pathFileSigI,
  ...
  if (m_readFileSigI) {
    ...
    if (barker == 0xE240) {
      ...
      m_readFileSigI.read(reinterpret_cast<char *>(&sizeofType),
                          sizeof sizeofType);
      m_typeSignalI = static_cast<TypeSignal>(sizeofType);

      unsigned short doubleHash;
      m_readFileSigI.read(reinterpret_cast<char *>(&doubleHash),
                          sizeof(doubleHash));

      if (doubleHash ==
          static_cast<unsigned short>(typeid(double).hash_code())) {
        return true;
      }
    }
  }
...
bool ReadComplexModule::ReadServiceParametersInBinSigQ() {
  m_readFileSigQ.open(m_pathFileSigQ,
                      std::ios_base::binary | std::ios_base::in);
  if (m_readFileSigQ) {
    unsigned short barker;
    ...
    if (barker == 0xE240) {
      ...
      unsigned short sizeofType;
      m_readFileSigQ.read(reinterpret_cast<char *>(&sizeofType),
                          sizeof sizeofType);
      m_typeSignalQ = static_cast<TypeSignal>(sizeofType);

      unsigned short doubleHash;
      m_readFileSigQ.read(reinterpret_cast<char *>(&doubleHash),
                          sizeof(doubleHash));
