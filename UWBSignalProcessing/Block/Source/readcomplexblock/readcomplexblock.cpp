...
ReadComplexBlock::ReadComplexBlock() : SourceBlock("ReadComplex") {
...
AbstractModule *ReadComplexBlock::GetModule() const { return m_module.get(); }

QString ReadComplexBlock::GetType() const { return "ReadComplexBlock"; }

void ReadComplexBlock::Change() {
  if (m_dialog && m_module) {
    m_module->SetPathFile(m_dialog->GetPathFileSigI(),
                          m_dialog->GetPathFileSigQ());
  }
}

void ReadComplexBlock::paint(QPainter *painter,
...
AbstractBlock *PluginBlock::LoudBlock() const { return new ReadComplexBlock; }

void ReadComplexBlock::RegistrOperationManager(
