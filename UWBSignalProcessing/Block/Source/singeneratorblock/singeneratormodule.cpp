...
SinGeneratorModule::SinGeneratorModule()
    : SourceModule<csignal<double>>(static_cast<uint8_t>(1)), m_lenSignal(2048),
      m_frequency(2 * M_PI * 1E+6), m_amplitude(1), m_phase(0),
      m_sampleTime(2E-9), m_generatorArray(m_lenSignal), m_az(0), m_frame(0) {
  StartGenerator();
  m_frame = 0;
...
void SinGeneratorModule::ChangeParametersGenerator(double frequency,
                                                   double amplitude,
                                                   double phase,
                                                   bool sampleTime) {
...
void SinGeneratorModule::TimerTimout() { QueueOperation(); }
