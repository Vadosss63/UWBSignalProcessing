...
SinGeneratorBlock::SinGeneratorBlock() : SourceBlock("SinGen") {
...
AbstractModule *SinGeneratorBlock::GetModule() const { return m_module.get(); }

QString SinGeneratorBlock::GetType() const { return "SinGeneratorBlock"; }

void SinGeneratorBlock::paint(QPainter *painter,
...
void SinGeneratorBlock::DrawImage(QPainter *painter) {
  ...
  QFont f("Times", 20);
  QRect rectText(
      static_cast<int>(rect.left()), static_cast<int>(rect.top() + 10),
  ...
  path.cubicTo(rect.width() / 2 + 5, rect.bottom(), rect.width() / 2 + 8,
               (rect.height()) / 2 + 16, rect.width() / 2 + 11,
               (rect.height()) / 2 + 15);

  painter->drawPath(path);

  QVector<QLine> m_image(3);
  m_image[0] = QLine(static_cast<int>(rect.width()) / 2 - 12,
                     static_cast<int>(rect.bottom()) - 10,
                     static_cast<int>(rect.width()) / 2 + 7,
                     static_cast<int>(rect.height()) / 2 + 9);
  m_image[1] = QLine(static_cast<int>(rect.width()) / 2 + 1,
                     static_cast<int>(rect.bottom()) - 25,
                     static_cast<int>(rect.width()) / 2 + 7,
                     static_cast<int>(rect.height()) / 2 + 9);
  m_image[2] = QLine(static_cast<int>(rect.width()) / 2 + 5,
                     static_cast<int>(rect.bottom()) - 20,
                     static_cast<int>(rect.width()) / 2 + 7,
                     static_cast<int>(rect.height()) / 2 + 9);
...
AbstractBlock *PluginBlock::LoudBlock() const { return new SinGeneratorBlock; }

void SinGeneratorBlock::Change() {
  if (m_dialog && m_module)
    m_module->ChangeParametersGenerator(
        m_dialog->GetFrequency(), m_dialog->GetAmplitude(),
        m_dialog->GetPhase(), m_dialog->GetSignalSize());
}

void SinGeneratorBlock::RegistrOperationManager(
