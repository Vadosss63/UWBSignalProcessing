...
ADCServerBlock::ADCServerBlock() : SourceBlock("АЦП") {
  setToolTip(
      "Получет информацию от группы АЦП и\nотправляет ее на I и Q порты");
}

AbstractBlock *ADCServerBlock::Clone() const { return new ADCServerBlock; }

AbstractModule *ADCServerBlock::GetModule() const { return m_module.get(); }

QString ADCServerBlock::GetType() const { return "ADCServerBlock"; }
...
void ADCServerBlock::paint(QPainter *painter,
...
void ADCServerBlock::DrawImage(QPainter *painter) {
  ...
  for (short i = 25; i < sizeList; i += 38) {
    painter->drawText(static_cast<int>(rect.right()) - 12,
                      static_cast<int>(rect.top()) + i, "I");
    painter->drawText(static_cast<int>(rect.right()) - 15,
                      static_cast<int>(rect.top()) + i + 19, "Q");
  }
  int j = 0;
...
AbstractBlock *PluginBlock::LoudBlock() const { return new ADCServerBlock; }

void ADCServerBlock::RegistrOperationManager(
