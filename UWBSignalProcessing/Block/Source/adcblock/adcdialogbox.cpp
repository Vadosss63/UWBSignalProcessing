...
ADCServerDialogBox::ADCServerDialogBox(ADCModule *parent, QWidget *wparent)
    : AbstractDialogCommand(wparent), ui(new Ui::UDPServerDialogBox),
      m_module(parent), m_senderToUDP(m_IpIndex.toStdString(), 0x4060) {
  /// TODO проверить
...
ADCServerDialogBox::~ADCServerDialogBox() { WriteDelayADC(); }
...
void ADCServerDialogBox::ReadDelayADC() {
  ...
  if (!settings.allKeys().isEmpty()) {
    ...
    bool ok;
    m_delayADCBoard.push_back(
        settings.value("delay_1").toString().toInt(&ok, 16));
    m_delayADCBoard.push_back(
        settings.value("delay_2").toString().toInt(&ok, 16));
    m_delayADCBoard.push_back(
        settings.value("delay_3").toString().toInt(&ok, 16));
    m_delayADCBoard.push_back(
        settings.value("delay_4").toString().toInt(&ok, 16));
    m_delayADCBoard.push_back(
        settings.value("delay_5").toString().toInt(&ok, 16));
  }
  settings.endGroup();
}

void ADCServerDialogBox::MakeConnections() {
  connect(ui->workMode2RadioButton, SIGNAL(toggled(bool)),
          ui->adcNumberComboBox, SLOT(setEnabled(bool)));
  connect(ui->signalLengthDial, SIGNAL(valueChanged(int)), this,
          SLOT(ChangedSignalLength(int)));
  connect(ui->adcDelayDial, SIGNAL(valueChanged(int)), this,
          SLOT(ChangedADCDelay(int)));

  connect(ui->signalFrequency, SIGNAL(valueChanged(double)), this,
          SLOT(ChangedSignalFreaq(double)));

  connect(ui->ipSelector, SIGNAL(activated(QString)), this,
          SLOT(ShowDialogMassage(QString)));
  connect(ui->workMode2RadioButton, SIGNAL(toggled(bool)), this,
          SLOT(ChangedWorkMode(bool)));
  connect(ui->findADCPushButton, SIGNAL(clicked()), this, SLOT(CheckIPs()));

  connect(ui->delayADC_1, SIGNAL(editingFinished()), this,
          SLOT(SetupDelayADC()));
  ui->delayADC_1->setProperty("index", 0);
  connect(ui->delayADC_2, SIGNAL(editingFinished()), this,
          SLOT(SetupDelayADC()));
  ui->delayADC_2->setProperty("index", 1);
  connect(ui->delayADC_3, SIGNAL(editingFinished()), this,
          SLOT(SetupDelayADC()));
  ui->delayADC_3->setProperty("index", 2);
  connect(ui->delayADC_4, SIGNAL(editingFinished()), this,
          SLOT(SetupDelayADC()));
  ui->delayADC_4->setProperty("index", 3);
  connect(ui->delayADC_5, SIGNAL(editingFinished()), this,
          SLOT(SetupDelayADC()));
  ui->delayADC_5->setProperty("index", 4);

  ReciverUWBDialog *reciverUWBDialog = new ReciverUWBDialog(this);
  connect(ui->ModuleControl, SIGNAL(clicked(bool)), reciverUWBDialog,
          SLOT(show()));
...
void ADCServerDialogBox::SetValuesUI() {
  double signalFrequency =
      (m_currentValues.signalPeriod_L.TX_PERIOD +
       (m_currentValues.signalPeriod_H.TX_PERIOD << 8) + 1) *
      0.08;
  ...
  ui->workMode0RadioButton->setChecked(
      !(m_currentValues.ufosAllRegimeReg.WORK_MODE));
  ui->workMode2RadioButton->setChecked(
      m_currentValues.ufosAllRegimeReg.WORK_MODE & 0x2);
  ui->continuousRadioButton->setChecked(
      m_currentValues.ufosAllRegimeReg.SDRAM_MODE);
  ui->strobRadioButton->setChecked(
      !(m_currentValues.ufosAllRegimeReg.SDRAM_MODE));

  ui->outerClkRadioButton->setChecked(
      m_currentValues.ufosAllControlReg.EXT_CLK_EN);
  ui->innerClkRadioButton->setChecked(
      !(m_currentValues.ufosAllControlReg.EXT_CLK_EN));
  ...
  ui->innerAzimuthRadioButton->setChecked(
      m_currentValues.adcControlReg.TEST_BEAR);
  ui->outerAzimuthRadioButton->setChecked(
      !(m_currentValues.adcControlReg.TEST_BEAR));

  ui->adcNumberComboBox->setCurrentIndex(
      m_currentValues.ufosAllADCNumberPosition.CountRX);
...
bool ADCServerDialogBox::SetADCDelay() {
  ...
  WriteRegisterADC((uint8_t)ADC_ALL_RegisterRX_DELAY::ADDR::ADDRESS,
                   m_currentValues.adcDelay);
  GetStatus();
  return true;
}
...
bool ADCServerDialogBox::SetUFOSAllRegimeReg() {
  ...
  m_currentValues.ufosAllRegimeReg.WORK_MODE =
      ui->workMode2RadioButton->isChecked();
  m_currentValues.ufosAllRegimeReg.SDRAM_MODE =
      ui->continuousRadioButton->isChecked();
  WriteRegisterADC((uint8_t)ADC_ALL_RegisterWorkMode::ADDR::ADDRESS,
                   m_currentValues.ufosAllRegimeReg);
  GetStatus();
  return true;
}
...
bool ADCServerDialogBox::SetUFOSAllControlReg() {
  ...
  m_currentValues.ufosAllControlReg.EXT_CLK_EN =
      ui->outerClkRadioButton->isChecked();
  m_currentValues.ufosAllControlReg.NS1_EN = ui->nsModeCheckBox->isChecked();
  m_currentValues.ufosAllControlReg.DATA_X = 0;
  WriteRegisterADC((uint8_t)ADC_ALL_RegisterControl::ADDR::ADDRESS,
                   m_currentValues.ufosAllControlReg);
  GetStatus();
  return true;
}
...
bool ADCServerDialogBox::SetADCControlReg() {
  ...
  m_currentValues.adcControlReg.TEST_BEAR =
      ui->innerAzimuthRadioButton->isChecked();
...
bool ADCServerDialogBox::SetUFOSAllADCNumber() {
  m_currentValues.ufosAllADCNumberPosition.CountRX =
      (uint8_t)(ui->adcNumberComboBox->currentText().toUInt() / 32 - 1);
  WriteRegisterADC((uint8_t)ADC_ALL_RegisterCountRX::ADDR::ADDRESS,
                   m_currentValues.ufosAllADCNumberPosition);
  GetStatus();
  return true;
}
...
void ADCServerDialogBox::SendCommandAbstractModule() { WriteSettingToADC(); }
...
void ADCServerDialogBox::ChangedADCDelay(int value) {
  ui->adcDelayLcdNumber->display(static_cast<double>(value * 128 + 4) * 0.15 *
                                 4);
}
...
void ADCServerDialogBox::ShowDialogMassage(const QString &index) {
  if (SetSignalPeriod() | SetADCDelay() | SetSignalLength() |
      SetUFOSAllRegimeReg() | SetUFOSAllControlReg() | SetADCControlReg() |
      SetUFOSAllADCNumber()) {
...
void ADCServerDialogBox::SwitchADCOn() {
  if (!ui->ipSelector->count()) {
    ...
    msgBox.setInformativeText(
        QString("Список найденных АЦП пуст. "
...
void ADCServerDialogBox::InitOldValue() {
  m_currentValues.signalPeriod_H.adress =
      (uint8_t)RegisterTXPERIOD::ADDR::ADDRESS_H;
  m_currentValues.signalPeriod_L.adress =
      (uint8_t)RegisterTXPERIOD::ADDR::ADDRESS_L;
}

bool ADCServerDialogBox::CheckStatus() {
  bool resultCheck = true;
  resultCheck = resultCheck &&
                CheckData(m_statusADC.control, m_currentValues.adcControlReg);
  resultCheck = resultCheck && CheckData(m_statusADC.txPeriod[0],
                                         m_currentValues.signalPeriod_L);
  resultCheck = resultCheck && CheckData(m_statusADC.txPeriod[1],
                                         m_currentValues.signalPeriod_H);
  resultCheck = resultCheck &&
                CheckData(m_statusADC.lengthTX, m_currentValues.signalLength);
  resultCheck = resultCheck &&
                CheckData(m_statusADC.registerAF, m_currentValues.registerAF);
  return resultCheck;
}
...
QList<std::pair<QString, QVariant>> ADCServerDialogBox::GetSetting() {
  ...
  listSetting.append(std::make_pair("signalPeriod_H",
                                    m_currentValues.signalPeriod_H.TX_PERIOD));
  listSetting.append(std::make_pair("signalPeriod_L",
                                    m_currentValues.signalPeriod_L.TX_PERIOD));
  listSetting.append(
      std::make_pair("RX_MODE", (uint8_t)m_currentValues.registerAF.RX_MODE));
  listSetting.append(std::make_pair(
      "TEST_BEAR", (uint8_t)m_currentValues.adcControlReg.TEST_BEAR));
...
bool ADCServerDialogBox::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
  ...
  double signalFrequency =
      1000.0 / ((m_currentValues.signalPeriod_L.TX_PERIOD +
                 (m_currentValues.signalPeriod_H.TX_PERIOD << 8) + 1) *
                0.08);
  ui->signalLengthDial->setValue(m_currentValues.signalLength.LengthTX);
