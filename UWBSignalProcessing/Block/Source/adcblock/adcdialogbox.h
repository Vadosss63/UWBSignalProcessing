...
private:
    ...
    template <typename TypeData> void WriteRegisterADC(uint8_t addr, TypeData& data)
    {
        RegisterControlBoardsADC_ADDR registerControlBoardsADC_ADDR;
        registerControlBoardsADC_ADDR.SPI_ADDR = static_cast<uint8_t>(RegisterControlBoardsADC_ADDR::ADDR::ADDRESS_ADDR);
        ...
        registerControlBoardsADC_DATA.SPI_ADDR = static_cast<uint8_t>(RegisterControlBoardsADC_ADDR::ADDR::ADDRESS_DATA);
        registerControlBoardsADC_DATA.SPI_DATA_ADC = *(reinterpret_cast<uint8_t*>(&data));
        m_senderToUDP.SendData(registerControlBoardsADC_DATA);
    }

    template <typename TypeData>
    bool CheckData(uint8_t data, TypeData& regData)
    {
