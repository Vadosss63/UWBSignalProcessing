...
ADCModule::ADCModule(): SourceModule(0),
    m_reciverData("192.168.0.1", 0x4002)
{
    SetCountBoards(2);
    m_reciverData.AddCallback([&]
    {
        AddDatagram();
    });
}
...
void ADCModule::Operate()
{
    ...
    if(m_currentFrame % 4 != m_dataPackI[0].az % 4  || m_currentFrame % 4 != m_dataPackQ[0].az % 4)
    {
    ...
    for (size_t i = 0; i < m_countPackageForChanel; ++i)
    {
        for (size_t j = 0; j < m_countDiscretDatagram; ++j)
        {
            ...
            Isignal[i * m_countDiscretDatagram + j] = m_dataPackI[i].data[j] - m_trendI[indexShift];
            Qsignal[i * m_countDiscretDatagram + j] = m_dataPackQ[i].data[j] - m_trendQ[indexShift];
        }
    }
...
void ADCModule::DataSocketOpen()
{
    ...
    {
        std::cerr << "ADC SocketOpen"<< m_reciverData.GetErrorsMessage() << std::endl;
        StopedModule();
    }
}
...
void ADCModule::SetAutoTrend(bool autoTrend)
{
