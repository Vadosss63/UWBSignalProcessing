...
CommandExecute::CommandExecute(std::string name, COMMANDS command,
                               int countData)
    : m_name(name), m_com(command), m_countData(countData) {
  ResetAnswer();
  ResetRequest();
}
...
std::string CommandExecute::Name() const { return m_name; }

std::vector<uint8_t> CommandExecute::GetRequest() {
  return m_request.GetWord();
}

std::vector<uint8_t> &CommandExecute::GetAnswer() { return m_answer.GetWord(); }
...
std::string CommandExecute::GetError() { return m_error; }

bool CommandExecute::IsNoError() const { return m_error.empty(); }

void CommandExecute::SetError(const std::string &error) { m_error = error; }

bool CommandExecute::CheckCRC() const { return m_answer.CheckCRC(); }
