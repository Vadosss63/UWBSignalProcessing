...
void PTreeController::Start() { m_reciverUDP.Start(); }

void PTreeController::Stop() { m_reciverUDP.Stop(); }
...
void PTreeController::InitCommands() {
  ...
  AddCommand(
      new CommandExecute("SyncronizationTune", COMMANDS::SYNCRONIZATION, 1));
  ...
  AddCommand(new CommandExecute("GetStatusSynchronitionCommand",
                                COMMANDS::GET_STATUS_SYNCH, 1));
  AddCommand(new CommandExecute("AccumSynchronition", COMMANDS::ACCUM, 4));
  AddCommand(new CommandExecute("ControlScanSynchronition",
                                COMMANDS::CONTROL_SCANE, 4));

  AddCommand(
      new CommandExecute("RebootSynchronitionCommand", COMMANDS::REBOOT, 0));
...
void PTreeController::SendAnswer() {
  ...
  if (m_currentCommand->IsNoError()) {
    ...
    if (m_currentCommand->CheckCRC()) {
      ...
      if (data.size() == 4) {
        uint32_t data_32 = (uint32_t)data[0] + (uint32_t)(data[1] << 8) +
                           (uint32_t)(data[2] << 16) +
                           (uint32_t)(data[3] << 24);
