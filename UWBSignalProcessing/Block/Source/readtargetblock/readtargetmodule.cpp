...
//        m_readFile.read(reinterpret_cast<char*>(header), sizeof
//        (AutoDatagram::Header)); OutBuffer outBuffer(header->countTarget);
//        AutoDatagram::DataTarget* dataTarget = new AutoDatagram::DataTarget;
//        for (size_t i = 0; i < header->countTarget; ++i) {
//            m_readFile.read(reinterpret_cast<char*>(dataTarget), sizeof
//            (AutoDatagram::DataTarget)); outBuffer[i].amp =
//            static_cast<double>(dataTarget->amp);

//            outBuffer[i].distence = static_cast<int>(dataTarget->distence /
//            outBuffer[i].resulution); outBuffer[i].direction =
//            static_cast<double>(dataTarget->direction);
//            outBuffer[i].startDistence =
//            static_cast<int>(dataTarget->startDistence /
//            outBuffer[i].resulution); outBuffer[i].stopDistence =
//            static_cast<int>(dataTarget->stopDistence /
//            outBuffer[i].resulution);
...
//            if(speed < 0)
//                speedInt = -countSpeed * speed /(3.6 * outBuffer[i].maxSpeed)
//                + 2 * countSpeed;
//            else
...
void ReadTargetModule::StartEvent() { SetPathFile(m_pathFile); }
