...
AbstractBlock* ReadTargetBlock::Clone() const
{
    return new ReadTargetBlock();
}

AbstractModule* ReadTargetBlock::GetModule() const
{
    return m_module.get();
}

QString ReadTargetBlock::GetType() const
{
    return "ReadTargetBlock";
}
...
void ReadTargetBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ReadTargetBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
