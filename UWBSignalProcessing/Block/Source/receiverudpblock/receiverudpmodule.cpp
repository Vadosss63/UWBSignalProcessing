...
ReceiverUDPModule::ReceiverUDPModule() :
    SourceModule<OutBuffer>(1), m_client("192.168.0.10", 12244)/*m_client("127.0.0.1", 12244)*/
{
    m_client.AddCallback([&]()
    {
        Operate();
    });
...
void ReceiverUDPModule::Operate()
{
    ...
    PushToOutput(std::make_pair(static_cast<csignal<double>>(sig1), static_cast<csignal<double>>(sig2)));
}
...
void ReceiverUDPModule::StartEvent()
{
    StartThread();
}
