...
GaussianNoiseSourceBlock::GaussianNoiseSourceBlock():
    SourceBlock("Noise")
{
...
void GaussianNoiseSourceBlock::Change()
{
    if(m_module && m_dialog)
        m_module->SetParameters(m_dialog->GetSignalSize(), m_dialog->GetDeviation());
}
...
void GaussianNoiseSourceBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void GaussianNoiseSourceBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
