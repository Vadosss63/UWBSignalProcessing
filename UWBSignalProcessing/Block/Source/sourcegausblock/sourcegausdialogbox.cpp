...
void SourceGausDialogBox::CreateSTWidget(QGridLayout* layoutSampleTime)
{
    ...
    connect(m_comboBoxTS, SIGNAL(currentIndexChanged(int)), this, SLOT(SetMaxRangeFrequecy(int)));
}
...
int SourceGausDialogBox::GetSigma()
{
    return m_sigma->getDisplayIntVal();
}
...
QList<std::pair<QString, QVariant> > SourceGausDialogBox::GetSetting()
{
    ...
    listSetting.append(std::make_pair("m_amplitude", m_amplitude->getDisplayIntVal()));
    listSetting.append(std::make_pair("m_sigma",m_sigma->getDisplayIntVal()));
    listSetting.append(std::make_pair("m_expectation",m_expectation->getDisplayIntVal()));
    return listSetting;

}

bool SourceGausDialogBox::SetSetting(QList<std::pair<QString, QVariant> > listSetting)
{
