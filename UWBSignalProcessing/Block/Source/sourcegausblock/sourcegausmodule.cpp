...
SourceGausModule::SourceGausModule() :
    SourceModule< csignal<double>>(static_cast<uint8_t>(1)), m_generatorArray(2048), m_lenSignal(2048),
    m_amplitude(200), m_sigma(3), m_expectation(500), m_az(0)
{
    StartGenerator();
    m_frame = 0;
}
...
void SourceGausModule::StartGenerator()
{
    ...
    {
        m_generatorArray[i] = m_amplitude*exp(-(pow((i - m_expectation), 2)) / (pow(m_sigma,2 )*2)) ;
    }
}

void SourceGausModule::ChangeParametersGenerator(double amplitude, size_t lenSignal, double sigma, double expectation)
{
...
void SourceGausModule::TimerTimout()
{
    QueueOperation();
}
