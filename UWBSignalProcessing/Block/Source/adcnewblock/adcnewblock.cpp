...
AbstractBlock* ADCNewBlock::Clone() const
{
    return new ADCNewBlock();
}

AbstractModule* ADCNewBlock::GetModule() const
{
    return m_module;
}

QString ADCNewBlock::GetType() const
{
    return "ADCNewBlock";
}
...
void ADCNewBlock::Change()
{
    if (m_module && m_dialog) {
        ...
        if(m_dialog->isImitationMode()){
            m_module->SetImitationMode(m_dialog->GetX(),
                                       m_dialog->GetY(),
                                       m_dialog->GetV());
...
void ADCNewBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ADCNewBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
