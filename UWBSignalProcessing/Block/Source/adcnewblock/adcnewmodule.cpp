...
ADCNewModule::ADCNewModule() : SourceModule(1) {
...
bool ADCNewModule::SendSetMod() {
  return m_adcController.AdcSetMode(m_sync_mode, m_adc_mode, m_samp_num,
                                    m_aver_num, m_aver_mode);
}

void ADCNewModule::SetAdcMode(ADC_SYNC sync_mode, ADC_MODE adc_mode,
...
void ADCNewModule::SetSyncMode() {
  if (m_isNormalMod) {
    ...
    m_adcController.SetImitationMode(m_imitationTarrget.x, m_imitationTarrget.y,
                                     m_imitationTarrget.v, m_isOneRun);
  }
}

void ADCNewModule::SetIsOneRun(bool isOneRun) { m_isOneRun = isOneRun; }
...
void ADCNewModule::SetFormatAngel(int stepAngel) { m_stepAngel = stepAngel; }

void ADCNewModule::SetNormalMode() { m_isNormalMod = true; }

ConnectProtocol::STATUS_HARDWARE ADCNewModule::GetStatusHardware() {
  return m_adcController.GetStatusHardware();
}

void ADCNewModule::SetShiftQ(int8_t shift) { m_shiftQ = shift; }
