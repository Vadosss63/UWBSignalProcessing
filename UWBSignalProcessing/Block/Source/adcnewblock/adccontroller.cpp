...
AdcController::AdcController() { StartThread(); }

AdcController::~AdcController() { StopThread(); }

bool AdcController::AdcStart() {
  AdcStartPackage package;
  package.command =
      static_cast<uint8_t>(ADC_COMMANDS::ADC_CMD_START); // Command
  package.runAdc = static_cast<uint8_t>(
      ADC_START_STOP_CMD::ADC_START_COMMAND); // Start/Stop byte
...
bool AdcController::AdcStop() {
  if (!StopSync())
    return false; /// TODO вернуть назад

  AdcStartPackage package;
  package.command =
      static_cast<u_int8_t>(ADC_COMMANDS::ADC_CMD_START); // Command
  package.runAdc = static_cast<u_int8_t>(
      ADC_START_STOP_CMD::ADC_STOP_COMMAND); // Start/Stop byte
...
bool AdcController::AdcSetMode(ADC_SYNC sync_mode, ADC_MODE adc_mode,
                               SAMPLE_NUM samp_num, AVER_NUM aver_num,
                               AVER_MODE aver_mode) {
  SetAdcModePackage package;
  package.command =
      static_cast<uint8_t>(ADC_COMMANDS::ADC_CMD_SET_MODE); // Command
...
void AdcController::SetImitationMode(int x, int y, int v, bool isOnenRun) {
  int D = static_cast<int>(
      std::round(static_cast<double>(std::sqrt(x * x + y * y)) / 0.0075) * 2);

  int tAntenna =
      y ? static_cast<int>(std::round(((0.290 * x) / static_cast<double>(y))) /
                           0.0075)
        : 0;
...
void AdcController::SetNormalMode() { StartImCMD(); }
...
ConnectProtocol::STATUS_HARDWARE AdcController::GetStatusHardware() {
  ...
  if (m_server->ReadWithTimeout(boost::posix_time::seconds(3))) {
    //        UART_UWB::CMD_HARDWARE_answer answer =
    //        m_server->GetData<UART_UWB::CMD_HARDWARE_answer>();
    auto ptrData = m_server->GetData_uint8();
    UART_UWB::CMD_HARDWARE_answer answer =
        *reinterpret_cast<UART_UWB::CMD_HARDWARE_answer *>(ptrData.first);
    ...
    sh.synchronizationStatus =
        static_cast<ConnectProtocol::SynchronizationStatus>(answer.sync);
