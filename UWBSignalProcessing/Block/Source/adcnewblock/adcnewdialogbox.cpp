...
QList<std::pair<QString, QVariant>> ADCNewDialogBox::GetSetting() {
  ...
  listSetting.append(
      std::make_pair("m_syncComboBox", m_syncComboBox->currentIndex()));
  listSetting.append(
      std::make_pair("m_adcModeComboBox", m_adcModeComboBox->currentIndex()));
  listSetting.append(std::make_pair("m_sampleNumComboBox",
                                    m_sampleNumComboBox->currentIndex()));
  listSetting.append(
      std::make_pair("m_averSigComboBox", m_averSigComboBox->currentIndex()));
  listSetting.append(
      std::make_pair("m_averSigGroupBox", m_averSigGroupBox->isChecked()));
  listSetting.append(std::make_pair("m_agelFormatComboBox",
                                    m_agelFormatComboBox->currentIndex()));
  listSetting.append(
      std::make_pair("m_shiftQSpinBox", m_shiftQSpinBox->value()));
  listSetting.append(
      std::make_pair("m_shiftLevelISpinBox", m_shiftLevelISpinBox->value()));
  listSetting.append(
      std::make_pair("m_shiftLevelQSpinBox", m_shiftLevelQSpinBox->value()));
  listSetting.append(
      std::make_pair("isImitationMode", m_imitatorWidget->isImitationMode()));

  return listSetting;
}

bool ADCNewDialogBox::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
...
int ADCNewDialogBox::GetX() const { return m_imitatorWidget->GetX(); }

int ADCNewDialogBox::GetY() const { return m_imitatorWidget->GetY(); }

int ADCNewDialogBox::GetV() const { return m_imitatorWidget->GetV(); }
...
bool ADCNewDialogBox::isOneRun() const { return m_imitatorWidget->isOneRun(); }
...
void ADCNewDialogBox::SetupAdcMode(int row, int column) {
  ...
  m_adcModeComboBox->addItem("Нормальный",
                             static_cast<int>(ADC_MODE::ADC_NORM_MODE));
  m_adcModeComboBox->addItem("Тестовые данные",
                             static_cast<int>(ADC_MODE::ADC_TPTN_MODE));
  m_adcModeComboBox->addItem("Sine режим",
                             static_cast<int>(ADC_MODE::ADC_SINE_MODE));
  m_adcModeComboBox->addItem("Powerdown",
                             static_cast<int>(ADC_MODE::ADC_PD_MODE));
...
void ADCNewDialogBox::SetupSyncMode(int row, int column) {
  ...
  m_syncComboBox->addItem("Внутренняя",
                          static_cast<int>(ADC_SYNC::SYNC_MODE_INT));
  m_syncComboBox->addItem("Тестовая",
                          static_cast<int>(ADC_SYNC::SYNC_MODE_TST));
...
ImitatorWidget::ImitatorWidget() { CreateWidget(); }

ImitatorWidget::~ImitatorWidget() {}

int ImitatorWidget::GetX() const { return m_x->value(); }

int ImitatorWidget::GetY() const { return m_y->value(); }

int ImitatorWidget::GetV() const { return m_v->value() /* / 3.6*/; }

bool ImitatorWidget::isImitationMode() const {
  return m_imitatorParametr->isChecked();
}

bool ImitatorWidget::isOneRun() const { return m_isOneRun->isChecked(); }
