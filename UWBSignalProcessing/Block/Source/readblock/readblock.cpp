...
AbstractModule* ReadBlock::GetModule() const
{
    return m_module.get();
}

AbstractBlock* ReadBlock::Clone() const
{
    return new ReadBlock;
}

QString ReadBlock::GetType() const
{
    return "ReadBlock";
}
...
void ReadBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ReadBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
