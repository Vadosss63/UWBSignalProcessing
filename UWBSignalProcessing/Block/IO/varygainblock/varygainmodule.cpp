...
VaryGainModule::VaryGainModule():
    IOModule(1, 1){}

void VaryGainModule::SetIndexConst(size_t index)
{
    m_index_const = index;
}
...
void VaryGainModule::Operate()
{
    ...
    for (size_t i = m_index_R2; i < m_index_const; ++i)
        temp[i] *= (std::pow(static_cast<double>(i) , 2) / std::pow(static_cast<double>(m_index_const), 2));
