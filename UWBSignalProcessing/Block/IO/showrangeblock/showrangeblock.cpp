...
ShowRangeBlock::ShowRangeBlock():
    IOBlock("Range")
{
    setToolTip("выводит все углы на одной дальности");
}

AbstractBlock* ShowRangeBlock::Clone() const
{
    return new ShowRangeBlock();
}

AbstractModule* ShowRangeBlock::GetModule() const
{
    return m_module.get();
}

QString ShowRangeBlock::GetType() const
{
    return "ShowRangeBlock";
}
...
void ShowRangeBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ShowRangeBlock::ChangeCountPorts()
{
    CreateBlockPorts();
}
...
void ShowRangeBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
