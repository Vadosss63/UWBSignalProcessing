...
ControlFramesBlock::ControlFramesBlock() :
    IOBlock("Контроль\n кадров"){}
...
AbstractModule* ControlFramesBlock::GetModule() const
{
    return m_module.get();
}

QString ControlFramesBlock::GetType() const
{
    return "ControlFramesBlock";
}

void ControlFramesBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ControlFramesBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
