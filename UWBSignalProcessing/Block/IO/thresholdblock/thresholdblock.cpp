...
{
    setToolTip("Ограничиваеит сигнал входной функциеей\nпервый порт ограничивающая функция\nвторой порт сигнал");
}

AbstractBlock* ThresholdBlock::Clone() const
{
    return new ThresholdBlock();
}

AbstractModule* ThresholdBlock::GetModule() const
{
    return m_module.get();
}

QString ThresholdBlock::GetType() const
{
    return "ThresholdBlock";
}

void ThresholdBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ThresholdBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
