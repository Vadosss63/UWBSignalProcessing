...
ThresholdConstBlock::ThresholdConstBlock():
    IOBlock("Пророг const")
{
    setToolTip("Опускает сигнал на знаяения парога/nи все что ниже нуля обнуляет");
}
...
QString ThresholdConstBlock::GetType() const
{
    return "ThresholdConstBlock";
}
...
void ThresholdConstBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ThresholdConstBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
