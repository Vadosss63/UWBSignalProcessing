...
LogicBlock::LogicBlock() :
    IOBlock("Logic")
{
    setToolTip("Принимет на вход два сигнала, выдает то первый пропуская второй, то второй пропуская первый");
}

AbstractBlock* LogicBlock::Clone() const
{
    return new LogicBlock();
}

AbstractModule* LogicBlock::GetModule() const
{
    return m_module.get();
}

QString LogicBlock::GetType() const
{
    return "LogicBlock";
}

void LogicBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void LogicBlock::ChangeCountPorts()
{
    CreateBlockPorts();
}
...
void LogicBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
