...
{
    setToolTip("Выполняет выбор арефметического блока /nиз предложенного списка.");
}

AbstractBlock *ArithmeticBlock::Clone() const
{
    return new ArithmeticBlock();
}

AbstractModule* ArithmeticBlock::GetModule() const
{
    return m_module.get();
}

QString ArithmeticBlock::GetType() const
{
...
void ArithmeticBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ArithmeticBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
