...
void TransferFrequencyBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void TransferFrequencyBlock::DrawImage(QPainter *painter)
{
    ...
    painter->drawText(static_cast<int>(rect.right()) - 19, static_cast<int>(rect.top()) + 20, "Re");
    painter->drawText(static_cast<int>(rect.right()) - 19, static_cast<int>(rect.bottom()) - 10, "Im");
}
...
void TransferFrequencyBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
