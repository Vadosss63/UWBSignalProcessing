...
void TransferFrequencyModule::Operate() {
  ...
  std::transform(tempI.begin(), tempI.end(), tempQ.begin(), cSignal.begin(),
                 [](double a, double b) { return std::complex<double>(a, b); });

  tempQ.reset(tempQ.size());
  auto interQ = tempQ.begin();
  std::transform(
      cSignal.begin(), cSignal.end(), m_gen.begin(), tempI.begin(),
...
void TransferFrequencyModule::SetTransferFrequency(double transfreq,
                                                   size_t samp) {
  double sampfreq;
