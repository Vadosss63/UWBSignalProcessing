...
TwoGainModule::TwoGainModule() : IOModule(1, 1) {}
...
void TwoGainModule::StartEvent() { ResetParameters(); }
...
void TwoGainModule::Operate() {
  ...
  if (temp.size() <= m_indexOtherGainSignal) // проверям значение индекса на
                                             // размерность сигнала
  {
    ResetParameters();
    return;
  ...
  std::copy(m_firstSignal.begin(),
            m_firstSignal.begin() + m_indexOtherGainSignal, temp.begin());
  m_isFirst = true;
