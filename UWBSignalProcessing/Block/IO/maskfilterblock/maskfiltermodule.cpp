...
MaskFilterModule::MaskFilterModule():
    IOModule(static_cast<uint8_t>(1), static_cast<uint8_t>(1)), m_tempFFT(8192), m_sigTemp(8192), m_mask(8192, 1), m_expectedMutex(false)
{
    //Создание плана для прямого преобразования Фурье
    m_plan_fft_forward = fftw_plan_dft_r2c_1d(m_tempFFT.size(), (double*) &m_sigTemp[0], (fftw_complex*) &m_tempFFT[0], FFTW_ESTIMATE );
...
{ 
    ...
    if(m_expectedMutex.compare_exchange_strong(flag, true))
    {
        if(temp.size() == 8192)
        {
            ...
            std::transform(m_tempFFT.begin(), m_tempFFT.end(), m_mask.begin(), m_tempFFT.begin(),
