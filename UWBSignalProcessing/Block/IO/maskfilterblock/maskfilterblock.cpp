...
MaskFilterBlock::MaskFilterBlock() : IOBlock("MaskFilter") {
  setToolTip("Фильтрация сигнала при помощи налажения маски(size 8192) в "
             "частотной области");
}

AbstractBlock *MaskFilterBlock::Clone() const { return new MaskFilterBlock(); }

AbstractModule *MaskFilterBlock::GetModule() const { return m_module.get(); }

QString MaskFilterBlock::GetType() const { return "MaskFilterBlock"; }
...
void MaskFilterBlock::paint(QPainter *painter,
...
void MaskFilterBlock::DrawImage(QPainter *painter) {
  ...
  path.cubicTo(rect.width() / 2 + 5, rect.bottom(), rect.width() / 2 + 8,
               (rect.height()) / 2 + 16, rect.width() / 2 + 17,
               (rect.height()) / 2 + 15);
  painter->drawPath(path);

  path.moveTo(rect.width() / 2 - 18, (rect.height()) / 2);
  path.cubicTo(rect.width() / 2 - 4, rect.height() / 2 - 22,
               rect.width() / 2 - 4, (rect.height()) / 2, rect.width() / 2 - 1,
               (rect.height()) / 2 + 1);
  path.cubicTo(rect.width() / 2 + 5, rect.bottom() - 17, rect.width() / 2 + 8,
               (rect.height()) / 2 - 1, rect.width() / 2 + 17,
               (rect.height()) / 2 - 2);
  painter->drawPath(path);
  ...
  path.cubicTo(rect.width() / 2 + 5, rect.bottom() - 34, rect.width() / 2 + 8,
               (rect.height()) / 2 - 18, rect.width() / 2 + 17,
               (rect.height()) / 2 - 19);
  painter->drawPath(path);

  QVector<QLine> m_image(3);
  m_image[0] = QLine(static_cast<int>(rect.width()) / 2 - 6,
                     static_cast<int>(rect.bottom()) - 14,
                     static_cast<int>(rect.width()) / 2 + 3,
                     static_cast<int>(rect.height()) / 2 + 13);
  m_image[1] = QLine(static_cast<int>(rect.width()) / 2 - 6,
                     static_cast<int>(rect.bottom()) - 48,
                     static_cast<int>(rect.width()) / 2 + 3,
                     static_cast<int>(rect.height()) / 2 - 21);
...
AbstractBlock *PluginBlock::LoudBlock() const { return new MaskFilterBlock; }

void MaskFilterBlock::RegistrOperationManager(
