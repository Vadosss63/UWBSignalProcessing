...
AbstractBlock *MathBlock::Clone() const
{
    return new MathBlock();
}

AbstractModule* MathBlock::GetModule() const
{
    return m_module.get();
}

QString MathBlock::GetType() const
{
    return "MathBlock";
}
...
void MathBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void MathBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
