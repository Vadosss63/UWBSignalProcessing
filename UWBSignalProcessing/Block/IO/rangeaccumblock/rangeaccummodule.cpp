...
RangeAccumModule::~RangeAccumModule() { ClearPlan(); }

void RangeAccumModule::SetSetting(size_t countAcumm, size_t startRange,
...
void RangeAccumModule::SetIsVARU(bool isVARU) { m_isVARU = isVARU; }
...
void RangeAccumModule::SetRange(size_t range) { m_range = range; }
...
void RangeAccumModule::ResetSignal() { m_currentAccum = 0; }
...
void RangeAccumModule::VARU() {
  for (size_t range = 0; range < plan_fft.size(); ++range) {
    double varu =
        (std::pow((double)range, 2) / std::pow((double)plan_fft.size(), 2));
...
void RangeAccumModule::NonThreshol(std::vector<std::valarray<double>> &data,
                                   size_t i) {
...
void RangeAccumModule::BinThreshol(std::vector<std::valarray<double>> &data,
                                   size_t i) {
  size_t indexData = m_range - i - 1;
  data[indexData].resize(m_countAcumm);
  // выполняем вычисление модуля
...
void RangeAccumModule::IndicatingThreshol(
    std::vector<std::valarray<double>> &data, size_t i) {
  size_t indexData = m_range - i - 1;
  data[indexData].resize(m_countAcumm);
  // выполняем вычисление модуля
  size_t indexSpeed = 0;
...
void RangeAccumModule::IndicatingAdapteThreshol(
    std::vector<std::valarray<double>> &data, size_t i) {
  size_t indexData = m_range - i - 1;
  data[indexData].resize(m_countAcumm);
  // выполняем вычисление модуля
...
void RangeAccumModule::SetGain(double gain) { m_gain = gain; }
...
void RangeAccumModule::SetPlan() {
  ...
  if (m_accumSignal.size() == m_range &&
      m_accumSignal.front().size() == m_countAcumm)
    return;
  ...
  for (size_t i = 0; i < m_range; ++i) {
    ...
    plan_fft[i] = fftw_plan_dft_1d(
        static_cast<int>(m_countAcumm), (fftw_complex *)&m_accumSignal[i][0],
