...
RangeAccumDialogBox::RangeAccumDialogBox() { CreateWidget(); }
...
QList<std::pair<QString, QVariant>> RangeAccumDialogBox::GetSetting() {
  ...
  listSetting.append(
      std::make_pair("thresholType", m_thresholType->currentIndex()));
...
bool RangeAccumDialogBox::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
...
int RangeAccumDialogBox::GetWindow() { return m_windows->currentIndex(); }

int RangeAccumDialogBox::GetThresholType() {
  return m_thresholType->currentIndex();
}

bool RangeAccumDialogBox::IsVARU() { return m_varu->isChecked(); }

double RangeAccumDialogBox::GetGain() { return m_gain->value(); }
