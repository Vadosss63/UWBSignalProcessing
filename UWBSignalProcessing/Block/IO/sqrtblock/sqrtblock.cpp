...
SqrtBlock::SqrtBlock() :
    IOBlock("SQRT"), m_module(nullptr)
{
    setToolTip("Вычисляет модуль комплексного числа");
}

AbstractBlock* SqrtBlock::Clone() const
{
    return new SqrtBlock();
}

AbstractModule* SqrtBlock::GetModule() const
{
    return m_module.get();
}

QString SqrtBlock::GetType() const
{
    return "SqrtBlock";
}
...
void SqrtBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void SqrtBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
