#ifndef AVTOELECTRONICAMODULE_H
#define AVTOELECTRONICAMODULE_H
...
class FieldZone {
  ...
  friend class AvtoelectronicaModule;
...
private:
  ...
  // Выполняет поиск целей с заданными скоростями
  void CheckTarget(size_t speed);
  // Проверяет наличие целей в наборе данных
  void FindTarget();

  // Получение значенич азимута с определенной дальности
  inline double GetAzimutTargets(size_t range, size_t speed) const;
  inline double GetABSVal(size_t range, size_t speed,
                          size_t numberAntenna = 1) const;
  ...
  std::list<Target::Point> m_listTargets;
  // Список целей
  std::list<Target> m_target;
...
class AvtoelectronicaModule
    : public IOModule<std::pair<csignal<double>, csignal<double>>, OutBuffer> {
  // Класс для тестирование модуля
  friend class Test_AvtoelectronicaModule;

public:
  AvtoelectronicaModule();
  ~AvtoelectronicaModule() override = default;
  ...
  void SetSetting(size_t countAcumm, size_t startRange, size_t range,
                  double threshol, double thresholDynamic);
