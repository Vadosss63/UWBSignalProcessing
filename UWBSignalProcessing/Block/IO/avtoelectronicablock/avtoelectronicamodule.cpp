...
void AvtoelectronicaModule::SetSetting(size_t countAcumm, size_t startRange,
...
std::vector<Target> FieldZone::Execute() {
  ...
  for (Target &tar : m_target) {
    ...
    auto it = std::remove_if(tar.GetPoints().begin(), tar.GetPoints().end(),
                             [this](Target::Point &p) {
                               double value =
                                   GetABSVal(static_cast<size_t>(p.distence),
                                             static_cast<size_t>(p.speed), 2);
                               if (value < m_threshol)
                                 return true;
...
void FieldZone::CheckTarget(size_t speed) {
  ...
  for (size_t range = 0; range < m_range; ++range) {
    ...
    {
      if (amp > m_threshol)
        m_listTargets.emplace_back(static_cast<int>(speed),
                                   static_cast<int>(range), amp);
...
void AvtoelectronicaModule::SetGain(double gain) { m_farField.m_gain = gain; }
...
void AvtoelectronicaModule::SetRangeLook(int xBegin, int xEnd, int yBegin,
                                         int yEnd) {
  m_xBegin = xBegin;
  m_xEnd = xEnd;
  m_yBegin = yBegin;
