...
Correlationmodule::Correlationmodule()
    : IOModule<csignal<double>, csignal<double>>(2, 1) {
  GeneratePlan(2048);
}
...
void Correlationmodule::Operate() {
  ...
  size_t size =
      temp.size() > temp2.size() ? temp.size() * 2 - 1 : temp2.size() * 2 - 1;
  ...
  std::transform(m_res1FFT.begin(), m_res1FFT.end(), m_res2FFT.begin(),
                 m_res1FFT.begin(),
  ...
  std::transform(m_Seq1.begin(), m_Seq1.end(), temp.begin(),
                 [&size](double res1) { return res1 / size; });
...
void Correlationmodule::GeneratePlan(size_t size) {
  ...
  m_plan_fft_forwardSeq1 =
      fftw_plan_dft_r2c_1d(m_Seq1.size(), (double *)&m_Seq1[0],
                           (fftw_complex *)&m_res1FFT[0], FFTW_ESTIMATE);
  //Создание плана для прямого преобразования Фурье
  m_plan_fft_forwardSeq2 =
      fftw_plan_dft_r2c_1d(m_Seq2.size(), (double *)&m_Seq2[0],
                           (fftw_complex *)&m_res2FFT[0], FFTW_ESTIMATE);
  //Создание плана для обратного преобразования Фурье
  m_plan_fft_backwardSeq1 =
      fftw_plan_dft_c2r_1d(m_res1FFT.size(), (fftw_complex *)&m_res1FFT[0],
                           (double *)&m_Seq1[0], FFTW_ESTIMATE);
}
