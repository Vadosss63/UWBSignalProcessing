...
CosTransferDialogBox::CosTransferDialogBox()
  ...
  connect(ui->silmpleFrequency500Mhz, SIGNAL(clicked()), this,
          SLOT(SetMaxRangeFrequecy500MHz()));
  connect(ui->silmpleFrequency2Hhz, SIGNAL(clicked()), this,
          SLOT(SetMaxRangeFrequecy2GHz()));
  //фиксация размеров окна
  setFixedHeight(sizeHint().height());
  setFixedWidth(sizeHint().width());
}

CosTransferDialogBox::~CosTransferDialogBox() { delete ui; }
...
QList<std::pair<QString, QVariant>> CosTransferDialogBox::GetSetting() {
  ...
  listSetting.append(std::make_pair("silmpleFrequency500Mhz",
                                    ui->silmpleFrequency500Mhz->isChecked()));
  listSetting.append(std::make_pair("dial", ui->dial->value()));
  listSetting.append(std::make_pair("sin", ui->sin->isChecked()));
  return listSetting;
}

bool CosTransferDialogBox::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
  ...
  foreach (auto parmSetting, listSetting) {
    ...
    if (parmSetting.first == "sin") {
      parmSetting.second.toBool() ? ui->sin->setChecked(true)
...
double CosTransferDialogBox::GetFrequency() { return ui->lcd->intValue(); }

bool CosTransferDialogBox::GetPhase() { return ui->sin->isChecked(); }
