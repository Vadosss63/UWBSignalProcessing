...
AccumPhaserModule::AccumPhaserModule() : IOModule(2, 2) {}
...
bool AccumPhaserModule::CheckAzimut(uint32_t azimuthI, uint32_t azimuthQ) {
  bool azimutTrue =
      (m_sigQ.azimuth() == azimuthQ) && (m_sigI.azimuth() == azimuthI);
  return azimutTrue;
}
...
void AccumPhaserModule::Operate() {
  ...
  if (m_countAccum == 1) {
    ...
    for (size_t i = 0; i < sizeSig; ++i) {
      m_expJ =
          std::exp(std::complex<double>(0, -m_phase * m_currentAccumalation));

      std::complex<double> temp =
          std::complex<double>(sigI[i], sigQ[i]) * m_expJ;
      sigI[i] = temp.real();
      sigQ[i] = temp.imag();
    }
