...
AbstractBlock* DiffBlock::Clone() const
{
    return new DiffBlock();
}

AbstractModule* DiffBlock::GetModule() const
{
    return m_module.get();
}

QString DiffBlock::GetType() const
{
    return "DiffBlock";
}
...
void DiffBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void DiffBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
