...
FFTmodule::FFTmodule():
    IOModule(static_cast<uint8_t>(1), static_cast<uint8_t>(2)), TempFFT(2048), m_sigTemp(2048)
{
    //Создание плана для прямого преобразования Фурье
    plan_fft_forward = fftw_plan_dft_r2c_1d(TempFFT.size(), (double*) &m_sigTemp[0], (fftw_complex*) &TempFFT[0], FFTW_ESTIMATE );
}

FFTmodule::~FFTmodule()
{
    fftw_destroy_plan(plan_fft_forward);
}
