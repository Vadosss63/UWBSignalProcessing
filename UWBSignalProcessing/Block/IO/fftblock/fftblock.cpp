...
FFTBlock::FFTBlock() :
    IOBlock("REtoMA"), m_module(nullptr)
{
    setToolTip("Делает преобразования сигнала\n к фазе и спектру");
}

AbstractBlock *FFTBlock::Clone() const
{
    return new FFTBlock();
}

AbstractModule* FFTBlock::GetModule() const
{
    return m_module.get();
}

QString FFTBlock::GetType() const
{
    return "FFTBlock";
}

void FFTBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void FFTBlock::DrawImage(QPainter *painter)
{
    ...
    painter->drawText(static_cast<int>(rect.left()) + 8, static_cast<int>(rect.center().y()) + 5, "Re");
    painter->drawText(static_cast<int>(rect.right()) - 18, static_cast<int>(rect.top()) + 20, "A");
    painter->drawText(static_cast<int>(rect.right()) - 25, static_cast<int>(rect.bottom()) - 10, "|M|");
...
void FFTBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
