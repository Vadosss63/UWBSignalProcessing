...
VRLISacanModule::VRLISacanModule() : IOModule(1, 1) { ResetScan(); }
...
void VRLISacanModule::SetThreshold(int threshold) { m_threshold = threshold; }
...
void VRLISacanModule::SendData() { CreateDataVRLI(); }
...
void VRLISacanModule::ClearTarget(size_t angel, size_t range, int val) {
  ...
  size_t endRange = (range + m_sigmaRange + 1) < m_scan[0].size()
                        ? (range + m_sigmaRange + 1)
                        : m_scan[0].size();

  size_t beginAngel = (angel < m_sigmaAngel) ? 0 : (angel - m_sigmaAngel);
  size_t endAngel = (angel + m_sigmaAngel + 1) < m_scan.size()
                        ? (angel + m_sigmaAngel + 1)
                        : m_scan.size();
...
void VRLISacanModule::AddTargetToScan(size_t angel, size_t range, int amp) {
  ...
  size_t endRange = (range + sigmaRange + 1) < m_scan[0].size()
                        ? (range + sigmaRange + 1)
                        : m_scan[0].size();

  size_t beginAngel = (angel < sigmaAngel) ? 0 : (angel - sigmaAngel);
  size_t endAngel = (angel + sigmaAngel + 1) < m_scan.size()
                        ? (angel + sigmaAngel + 1)
                        : m_scan.size();
...
bool VRLISacanModule::CheckColusion(const Target &target) {
  ...
  for (auto &checkTarget : m_targets) {
    ...
    int angelCheck =
        static_cast<int>(checkTarget.angel) - static_cast<int>(target.angel);
    int rangeCheck =
        static_cast<int>(checkTarget.range) - static_cast<int>(target.range);
...
double &VRLISacanModule::GetValScan(const size_t &angelIndex,
                                    const size_t &rangeIndex) {
  return m_scan[angelIndex][rangeIndex];
}

void VRLISacanModule::StartEvent() { ResetScan(); }
