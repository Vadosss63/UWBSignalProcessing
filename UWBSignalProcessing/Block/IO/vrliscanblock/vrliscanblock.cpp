...
AbstractBlock* VRLIScanBlock::Clone() const
{
    return new VRLIScanBlock();
}

AbstractModule* VRLIScanBlock::GetModule() const
{
    return m_module.get();
}

QString VRLIScanBlock::GetType() const
{
    return "VRLIScanBlock";
}
...
void VRLIScanBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void VRLIScanBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
