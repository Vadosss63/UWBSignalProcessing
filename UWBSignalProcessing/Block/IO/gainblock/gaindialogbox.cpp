...
GainDialogBox::GainDialogBox()
{
    ...
    connect(this, SIGNAL(ValueChanged(double)), m_QLCDNumber, SLOT(display(double)));
...
bool GainDialogBox::SetSetting(QList<std::pair<QString, QVariant> > listSetting)
{
    ...
    foreach (auto parmSetting, listSetting)
    {
        ...
        if(parmSetting.first == "m_QLCDNumber"){
            ...
            m_numderDouble = static_cast<uint8_t>(qAbs(parmSetting.second.toDouble()) - m_numderInt)* 10;
