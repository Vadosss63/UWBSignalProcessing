...
QList<std::pair<QString, QVariant>> SyntheticApertureDialogBox::GetSetting() {
  ...
  for (int i = 0; i < m_stopIndex.size(); ++i)
    listSetting.append(
        std::make_pair(QString("stopIndex%1").arg(i), m_stopIndex[i]->value()));

  return listSetting;
}

bool SyntheticApertureDialogBox::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
  ...
  for (int i = 0; i < m_stopIndex.size(); ++i) {
    ...
    if (listSetting[i].first == QString("stopIndex%1").arg(i)) {
      m_stopIndex[i]->setValue(
          static_cast<int>(listSetting[i].second.toDouble()));
    }
  }
...
void SyntheticApertureDialogBox::CreateWidget(int countShift) {
  ...
  m_mainLayout->addWidget(
      new QLabel("Индексы конца накопления для 2, 4, 8, 16"));
