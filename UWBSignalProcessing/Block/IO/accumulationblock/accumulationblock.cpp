...
AccumulationBlock::AccumulationBlock() : IOBlock("Накопление"), m_image(4) {
  ...
  m_image[0] = QLine(
      ...
      static_cast<int>(rect.right()) - 15, static_cast<int>(rect.top()) + 15);
  m_image[1] = QLine(static_cast<int>(rect.left()) + 15,
                     static_cast<int>(rect.top()) + 15,
                     static_cast<int>(rect.width() / 2) + 15,
                     static_cast<int>(rect.height() / 2));
  m_image[2] = QLine(static_cast<int>(rect.left()) + 15,
                     static_cast<int>(rect.bottom()) - 15,
                     static_cast<int>(rect.width() / 2) + 15,
                     static_cast<int>(rect.height() / 2));
  m_image[3] = QLine(static_cast<int>(rect.left()) + 15,
                     static_cast<int>(rect.bottom()) - 15,
                     static_cast<int>(rect.right()) - 15,
                     static_cast<int>(rect.bottom()) - 15);
}
...
AbstractModule *AccumulationBlock::GetModule() const { return m_module.get(); }

QString AccumulationBlock::GetType() const { return "ConstAccumulationBlock"; }
...
void AccumulationBlock::paint(QPainter *painter,
...
AbstractBlock *PluginBlock::LoudBlock() const { return new AccumulationBlock; }

void AccumulationBlock::RegistrOperationManager(
