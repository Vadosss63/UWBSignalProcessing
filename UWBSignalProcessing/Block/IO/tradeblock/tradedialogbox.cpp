...
TradeDialogBox::TradeDialogBox() {
  ...
  connect(m_countShift, SIGNAL(currentIndexChanged(int)), this,
          SLOT(ChangeCountShift(int)));
...
QList<std::pair<QString, QVariant>> TradeDialogBox::GetSetting() {
  ...
  listSetting.append(
      std::pair<QString, QVariant>("m_shiftSize", m_shiftBoards.size()));
  for (int i = 0; i < m_shiftBoards.size(); ++i)
    listSetting.append(
        std::make_pair(QString("m_shift%1").arg(i), m_shiftBoards[i]->value()));

  return listSetting;
}

bool TradeDialogBox::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
