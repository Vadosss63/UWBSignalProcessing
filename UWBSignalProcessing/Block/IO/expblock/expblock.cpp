...
AbstractBlock *ExpBlock::Clone() const
{
    return new ExpBlock();
}

AbstractModule* ExpBlock::GetModule() const
{
    return m_module.get();
}

QString ExpBlock::GetType() const
{
    return "ExpBlock";
}
...
void ExpBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void ExpBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
