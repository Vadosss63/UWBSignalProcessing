...
AvtoelectronicaRawModule::AvtoelectronicaRawModule() : IOModule(1, 1) {}

void AvtoelectronicaRawModule::SetSetting(size_t countAcumm, size_t startRange,
                                          size_t range, double threshol,
                                          double thresholDynamic) {
...
void FieldZone::SetCountAccum(size_t countAcumm) { m_countAcumm = countAcumm; }

void FieldZone::SetStartRange(size_t startRange) { m_startRange = startRange; }

void FieldZone::SetRange(size_t range) { m_range = range; }

void AvtoelectronicaRawModule::Operate() {
  ...
  bool isExecute1 =
      m_farField.m_countAcumm <= m_farField.m_currentAccumSignalSecondAntenna;
  bool isExecute2 =
      m_farField.m_countAcumm <= m_farField.m_currentAccumSignalFirstAntenna;
...
void AvtoelectronicaRawModule::StartEvent() {
  m_farField.ResetSignal();
...
void AvtoelectronicaRawModule::StopEvent() {
  std::cout << __PRETTY_FUNCTION__;
  std::cout << std::endl;
  IOModule::StopEvent();
}

void FieldZone::AccumSigFirstAntenna(
    std::pair<csignal<double>, csignal<double>> &sig) {
  if (m_currentAccumSignalFirstAntenna < m_countAcumm) {
    // выполняем накопление
    for (size_t range = m_startRange; range < (m_startRange + m_range);
         ++range) {
      double real = sig.first[range];
      double im = sig.second[range];
      size_t index = range - m_startRange;
      m_accumSignalFirstAntenna[index][m_currentAccumSignalFirstAntenna] =
          std::complex<double>(real, im);
    }
  }
}

void FieldZone::AccumSigSecondAntenna(
    std::pair<csignal<double>, csignal<double>> &sig) {
  if (m_currentAccumSignalSecondAntenna < m_countAcumm) {
    ...
    for (size_t range = m_startRange; range < (m_startRange + m_range);
      ...
      double im = sig.second[range];
      size_t index = range - m_startRange;
      m_accumSignalSecondAntenna[index][m_currentAccumSignalSecondAntenna] =
          std::complex<double>(real, im);
    }
  }
}
...
double FieldZone::MeanValue() const {
  double mean = 0;
  int size = 0;
  for (size_t range = m_mZona; range < m_accumSignalFirstAntenna.size();
       ++range) {
    for (size_t speed = 0; speed < m_accumSignalFirstAntenna[range].size();
         ++speed) {
...
size_t FieldZone::GetMyCurrentSpeed() const {
  ...
  for (size_t speed = 0; speed < speedSize; ++speed) {
    ...
    double size = (m_accumSignalFirstAntenna.size() - m_mZona) *
                  m_accumSignalFirstAntenna[0].size();
    deviation[speed] = std::sqrt(deviation[speed] / (size - 1));
  }

  size_t currentSpeed = static_cast<size_t>(
      std::max_element(deviation.begin(), deviation.end()) - deviation.begin());
  return currentSpeed;
}
...
void FieldZone::SetMZona(size_t mZona) { m_mZona = mZona; }

void FieldZone::SetMaxSpeed(double maxSpeed) { m_maxSpeed = maxSpeed; }
...
std::vector<std::vector<double>> FieldZone::ExecuteRAWData() {
  std::vector<std::vector<double>> data =
      std::vector<std::vector<double>>(256, std::vector<double>(512));
...
double FieldZone::GetABSVal(size_t range, size_t speed,
                            size_t numberAntenna) const {
...
double FieldZone::GetAbs(size_t range, size_t speed,
                         size_t numberAntenna) const {
  if (numberAntenna == 1) {
    double val =
        std::pow(std::real(m_accumSignalFirstAntenna[range][speed]), 2) +
  ...
  if (numberAntenna == 2) {
    double val =
        std::pow(std::real(m_accumSignalSecondAntenna[range][speed]), 2) +
...
void AvtoelectronicaRawModule::SetGain(double gain) {
  m_farField.m_gain = gain;
}

void AvtoelectronicaRawModule::SetGainDynamic(double gain) {
  m_farField.m_gainDynamic = gain;
}

void AvtoelectronicaRawModule::SetBaseAntenna(double value) {
  m_farField.SetBaseAntenna(value);
}

void AvtoelectronicaRawModule::SetMaxSpeed(double maxSpeed) {
  m_farField.SetMaxSpeed(maxSpeed);
}

void AvtoelectronicaRawModule::SetRangeLook(int xBegin, int xEnd, int yBegin,
                                            int yEnd) {
  m_xBegin = xBegin;
  m_xEnd = xEnd;
  m_yBegin = yBegin;
...
void AvtoelectronicaRawModule::SetIsVARU(bool isVARU) {
...
void FieldZone::SetPlan() {
  ...
  for (size_t i = 0; i < m_range; ++i) {
    ...
    plan_fftFirstAntenna[i] = fftw_plan_dft_1d(
        static_cast<int>(m_countAcumm),
        reinterpret_cast<fftw_complex *>(&m_accumSignalFirstAntenna[i][0]),
        reinterpret_cast<fftw_complex *>(&m_accumSignalFirstAntenna[i][0]),
        FFTW_FORWARD, FFTW_ESTIMATE);

    m_accumSignalSecondAntenna[i].resize(m_countAcumm);
    plan_fftSecondAntenna[i] = fftw_plan_dft_1d(
        static_cast<int>(m_countAcumm),
        reinterpret_cast<fftw_complex *>(&m_accumSignalSecondAntenna[i][0]),
        reinterpret_cast<fftw_complex *>(&m_accumSignalSecondAntenna[i][0]),
        FFTW_FORWARD, FFTW_ESTIMATE);
  }
}
