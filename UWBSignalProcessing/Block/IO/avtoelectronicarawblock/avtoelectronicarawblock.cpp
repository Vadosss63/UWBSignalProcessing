...
{
    setToolTip("Модуль формирования сырой радиолокационной информации автоэлектроники");
}
...
void AvtoelectronicaRawBlock::Change()
{
    if (m_module && m_dialog)
    {
        ...
        m_module->SetSetting(m_dialog->GetCountAcumm(),
                             m_dialog->GetNumberRange(),
...
void AvtoelectronicaRawBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void AvtoelectronicaRawBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
