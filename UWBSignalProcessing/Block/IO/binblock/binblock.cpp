...
BinBlock::BinBlock() :
    IOBlock("BinSig"), m_module(nullptr)
{
    setToolTip("Блок формирования бинарного сигнала");
}

AbstractBlock* BinBlock::Clone() const
{
    return new BinBlock();
}

AbstractModule* BinBlock::GetModule() const
{
    return m_module.get();
}

QString BinBlock::GetType() const
{
    return "BinBlock";
}
...
void BinBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void BinBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
