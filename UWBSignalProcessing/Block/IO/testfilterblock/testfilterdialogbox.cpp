...

{
    ...
    connect(ui->FilterType_comboBox, SIGNAL(activated(int)), this, SLOT(FilterDialogSet(int)));

    //Изменение количества выходов
    connect(ui->Port_spinBox, SIGNAL(valueChanged(int)),this, SLOT(NumberPorts()));
...
void TestFilterDialogBox::FilterDialogSet(const int &type)
{
    DialogSet(type);
}

void TestFilterDialogBox::NumberPorts()
{
    port_state = true;
}
...
int TestFilterDialogBox::GetOrder()
{
    return ui->OrderspinBox->value();
}

bool TestFilterDialogBox::GetPortState() const
{
    return port_state;
}
...
QList<std::pair<QString, QVariant> > TestFilterDialogBox::GetSetting()
{
    ...
    listSetting.append(std::make_pair("FilterType_comboBox",ui->FilterType_comboBox->currentIndex()));
    ...
    listSetting.append(std::make_pair("Sample_Freq_comboBox",  ui->Sample_Freq_comboBox->currentIndex()));
    listSetting.append(std::make_pair("FreqCut_1_SpinBox",  ui->FreqCut_1_SpinBox->value()));
    listSetting.append(std::make_pair("FreqCut_2_SpinBox",  ui->FreqCut_2_SpinBox->value()));
    return listSetting;
}

bool TestFilterDialogBox::SetSetting(QList<std::pair<QString, QVariant> >listSetting)
{
