...
TestFilterBlock::TestFilterBlock() :
    IOBlock("Filter")
{
    setToolTip("Выполняет фильтрацию входного сигнала");
}

AbstractBlock* TestFilterBlock::Clone() const
{
    return new TestFilterBlock();
}

AbstractModule* TestFilterBlock::GetModule() const
{
    return m_module.get();
}

QString TestFilterBlock::GetType() const
{
    return "TestFilterBlock";
}

void TestFilterBlock::Change()
{
    if (m_module && m_dialog) {
        m_module->SetFilterParameters(m_dialog->GetSampleFreq(),
                                      m_dialog->GetCutFreq1(),
                                      m_dialog->GetCutFreq2(),
                                      m_dialog->GetOrder(),
...
void TestFilterBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void TestFilterBlock::ChangeCountPorts()
{
    CreateBlockPorts();
}
...
void TestFilterBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
