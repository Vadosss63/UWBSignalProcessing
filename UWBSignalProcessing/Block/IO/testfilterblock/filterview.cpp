...
FilterDialogBox::FilterDialogBox() {
  ...
  SetTopLayout(layout);
  connect(m_selectState, SIGNAL(currentIndexChanged(int)), this,
...
QList<std::pair<QString, QVariant>> FilterDialogBox::GetSetting() {
  if (m_currentState) {
    QList<std::pair<QString, QVariant>> settings =
        m_currentState->SaveSettings();
    settings.push_front(std::pair<QString, QVariant>(
        "TypeOfFilter", QString(m_selectState->currentText())));
    return settings;
  }
...
bool FilterDialogBox::SetSetting(
...
int FilterDialogBox::GetFilterType() { return m_currentState->GetFilterType(); }
...
int SettingSignal::GetOrderFilter() { return m_orderFilter->value(); }
...
QList<std::pair<QString, QVariant>> SettingSignal::SaveSettings() {
  ...
  settings.push_back(
      std::make_pair("SamplingPeriod", m_sampUnit->currentText()));
  settings.push_back(std::make_pair("CountPorts", m_countPorts->value()));
  return settings;
}

void SettingSignal::SetParameter(
    const QList<std::pair<QString, QVariant>> &param) {
...
FilterState::FilterState(QWidget *wparent) : QWidget(wparent) {
  ...
  connect(m_buttonSettingSignal, SIGNAL(clicked(bool)), this,
          SLOT(ShowSettingFilter()));
}

FilterState::~FilterState() { delete m_settingSignal; }
...
QString FilterState::Name() { return QString(); }

void FilterState::ChangeState(FilterDialogBox *dialog, FilterState *state) {
  dialog->ChangeState(state);
}

SettingSignal *FilterState::GetSettingSignal() const { return m_settingSignal; }

double FilterState::GetCutFreq1() { return -1; }

double FilterState::GetCutFreq2() { return -1; }

int FilterState::GetFilterType() { return -1; }
...
Lowpass::Lowpass(QWidget *wparent) : FilterState(wparent) { CreateWidget(); }
...
QList<std::pair<QString, QVariant>> Lowpass::SaveSettings() const {
  ...
  settings.push_back(std::make_pair(
      "CutoffFrequency", QString::number(m_freqCut->getDisplayIntVal())));
...
QString Lowpass::Name() { return QString("ФНЧ"); }

int Lowpass::GetFilterType() { return 0; }

double Lowpass::GetCutFreq1() { return m_freqCut->getDisplayIntVal(); }

///=============Hightpass===============================///

Hightpass::Hightpass(QWidget *wparent) : FilterState(wparent) {
  CreateWidget();
}
...
QString Hightpass::Name() { return QString("ФВЧ"); }

int Hightpass::GetFilterType() { return 1; }

double Hightpass::GetCutFreq1() { return m_freqCut->getDisplayIntVal(); }
...
Bandpass::Bandpass(QWidget *wparent) : FilterState(wparent) { CreateWidget(); }

QList<std::pair<QString, QVariant>> Bandpass::SaveSettings() const {
  QList<std::pair<QString, QVariant>> settings;
  settings.push_back(std::make_pair(
      "CutoffFrequency1", QString::number(m_freqCutFirst->getDisplayIntVal())));
  settings.push_back(
      std::make_pair("CutoffFrequency2",
                     QString::number(m_freqCutSecond->getDisplayIntVal())));
  QList<std::pair<QString, QVariant>> settingsInWindow;
  settingsInWindow = GetSettingSignal()->SaveSettings();
  settings = settings + settingsInWindow;
  return settings;
...
QString Bandpass::Name() { return QString("Полосовой фильтр"); }

double Bandpass::GetCutFreq1() { return m_freqCutFirst->getDisplayIntVal(); }

double Bandpass::GetCutFreq2() { return m_freqCutSecond->getDisplayIntVal(); }

int Bandpass::GetFilterType() { return 2; }
...
Bandstop::Bandstop(QWidget *wparent) : FilterState(wparent) { CreateWidget(); }
...
QString Bandstop::Name() { return QString("Режекторный фильтр"); }

int Bandstop::GetFilterType() { return 3; }

double Bandstop::GetCutFreq1() { return m_freqCutFirst->getDisplayIntVal(); }

double Bandstop::GetCutFreq2() { return m_freqCutSecond->getDisplayIntVal(); }
...
Hlbrt::Hlbrt(QWidget *wparent) : FilterState(wparent) { CreateWidget(); }
...
QString Hlbrt::Name() { return QString("фильтр Гильберта"); }
...
int Hlbrt::GetFilterType() { return 4; }
