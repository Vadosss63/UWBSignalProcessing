...
void TestFilterModule::SetFilterParameters(double SampleFreq, double CutFreq1,
                                           double CutFreq2, int order,
                                           FilterType type) {
  auto freq_cut = 0.0;
  ...
  case FilterType::Lowpass:
    ...
    if (m_filter && (m_filter->GetFilterType() == FilterType::Lowpass)) {
      ...
      m_filter =
          std::unique_ptr<BaseFilter>(new LowPassFilter(order, freq_cut));
    }
  ...
  case FilterType::Hightpass:
    ...
    if (m_filter && (m_filter->GetFilterType() == FilterType::Hightpass)) {
      ...
      m_filter =
          std::unique_ptr<BaseFilter>(new HightPassFilter(order, freq_cut));
    }
    break;
  ...
  case FilterType::Bandpass:
    ...
    if (m_filter && (m_filter->GetFilterType() == FilterType::Bandpass)) {
      ...
      m_filter = std::unique_ptr<BaseFilter>(
          new BandPassFilter(order, low_freq_cut, hight_freq_cut));
    }
  ...
  case FilterType::Bandstop:
    ...
    if (m_filter && (m_filter->GetFilterType() == FilterType::Bandstop)) {
      ...
      m_filter = std::unique_ptr<BaseFilter>(
          new BandStopFilter(order, low_freq_cut, hight_freq_cut));
    }
...
TestFilterModule::BaseFilter::BaseFilter(size_t filterOrder)
    : m_filterOrder(filterOrder), m_b(filterOrder + 1), m_a(filterOrder + 1),
      m_spectrumBuilder(2048), m_signalBuilder(2048) {
  m_a[0] = 1;
}
...
void TestFilterModule::BaseFilter::FiltrationProcess(
    const std::vector<double> &inputSignal, std::vector<double> &outputSignal) {
  ...
  std::transform(sp1.begin(), sp1.end(), m_fil.begin(), m_sp3.begin(),
                 [](const std::complex<double> &s1,
                    const std::complex<double> &s2) { return s1 * s2; });

  outputSignal = m_signalBuilder.BuildReverseFFT(m_sp3);
}

void TestFilterModule::BaseFilter::SimpleFiltrationProcess(
    const std::vector<double> &inputSignal, std::vector<double> &outputSignal) {
  ...
  std::copy(sp3.begin(), sp3.begin() + inputSignal.size(),
            outputSignal.begin());
}
...
TestFilterModule::LowPassFilter::LowPassFilter(size_t filterOrder,
                                               double freqCut)
    : BaseFilter(filterOrder), m_freqCut(freqCut) {
  FilterInit();
}
...
void TestFilterModule::LowPassFilter::ChangeParams(size_t order,
                                                   double freq_cut1, double) {
  m_filterOrder = order;
  m_freqCut = freq_cut1;
...
void TestFilterModule::LowPassFilter::FilterInit() {
  ...
  std::for_each(
      m_b.begin(), m_b.end(),
      [&index, this](double &impulseResponseCoefficient) {
        if (index == m_filterOrder / 2)
          impulseResponseCoefficient = m_freqCut;
        else
          impulseResponseCoefficient =
              m_freqCut *
              (sin((M_PI * (index - (m_filterOrder / 2))) * m_freqCut) /
               ((M_PI * (index - (m_filterOrder / 2))) * m_freqCut));

        impulseResponseCoefficient =
            (0.54 - 0.46 * cos(2 * M_PI * index / m_filterOrder)) *
            impulseResponseCoefficient;
...
TestFilterModule::HightPassFilter::HightPassFilter(const size_t filterOrder,
                                                   const double freqCut)
    : BaseFilter(filterOrder), m_freqCut(freqCut) {
  FilterInit();
}

TestFilterModule::FilterType
TestFilterModule::HightPassFilter::GetFilterType() {
  return FilterType::Hightpass;
}

void TestFilterModule::HightPassFilter::ChangeParams(size_t order,
                                                     double freq_cut1, double) {
  m_filterOrder = order;
  m_freqCut = freq_cut1;
  m_b.resize(m_filterOrder + 1);
...
TestFilterModule::BandPassFilter::BandPassFilter(const size_t filterOrder,
                                                 const double lowFreqCut,
                                                 const double hightFreqCut)
    : BaseFilter(filterOrder), m_lowFreqCut(lowFreqCut),
...
void TestFilterModule::BandPassFilter::ChangeParams(size_t order,
                                                    double freq_cut1,
                                                    double freq_cut2) {
  m_filterOrder = order;
...
void TestFilterModule::BandPassFilter::FilterInit() {
  ...
  std::copy(res.begin() + m_b.size() / 2,
            res.begin() + m_b.size() / 2 + m_b.size(), m_b.begin());
}

TestFilterModule::BandStopFilter::BandStopFilter(const size_t filterOrder,
                                                 const double lowFreqCut,
                                                 const double hightFreqCut)
    : BaseFilter(filterOrder), m_lowFreqCut(lowFreqCut),
      m_hightFreqCut(hightFreqCut) {
...
void TestFilterModule::BandStopFilter::ChangeParams(size_t order,
                                                    double freq_cut1,
                                                    double freq_cut2) {
  m_filterOrder = order;
  m_lowFreqCut = freq_cut1;
...
void TestFilterModule::BandStopFilter::FilterInit() {
  ...
  std::transform(lowfilterCoeff.begin(), lowfilterCoeff.end(),
                 hightfilterCoeff.begin(), m_b.begin(),
                 [](double &l, double &h) { return (l + h); });
}

TestFilterModule::HilbertFilter::HilbertFilter() : BaseFilter() {}

TestFilterModule::FilterType TestFilterModule::HilbertFilter::GetFilterType() {
  return FilterType::Hilbert;
}

void TestFilterModule::HilbertFilter::FiltrationProcess(
    const std::vector<double> &inputSignal, std::vector<double> &outputSignal) {
  ...
  fftw_plan forwardFFTplan = fftw_plan_dft_1d(
      FFTOfAnalyticSignal.size(), (fftw_complex *)&analyticSignal[0],
      (fftw_complex *)&FFTOfAnalyticSignal[0], FFTW_FORWARD, FFTW_ESTIMATE);

  fftw_plan backwardFFTplan = fftw_plan_dft_1d(
      FFTOfAnalyticSignal.size(), (fftw_complex *)&FFTOfAnalyticSignal[0],
      (fftw_complex *)&analyticSignal[0], FFTW_BACKWARD, FFTW_ESTIMATE);
  ...
  std::transform(FFTOfAnalyticSignal.begin(), FFTOfAnalyticSignal.end(),
                 frequencyMask.begin(), FFTOfAnalyticSignal.begin(),
  ...
  std::transform(analyticSignal.begin(), analyticSignal.end(),
                 outputSignal.begin(),
...
void TestFilterModule::HilbertFilter::SimpleFiltrationProcess(
    const std::vector<double> &inputSignal, std::vector<double> &outputSignal) {
