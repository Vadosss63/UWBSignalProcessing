...
private:
    ...
    public:
        FFTBuilder(const size_t fftLength)
            : m_fftLength(fftLength),
              m_spectr(m_fftLength),
              m_complexSignal(m_fftLength),
              m_plan(nullptr)
        {
            InitPlan();
        }
        ...
        FFTBuilder(const FFTBuilder& rhs)
            : m_fftLength(rhs.m_fftLength),
              m_spectr(rhs.m_spectr),
              m_complexSignal(rhs.m_complexSignal),
              m_plan(nullptr)
        {
            InitPlan();
        }
        ...
        std::vector<std::complex<double>> BuildFFT(const std::vector<double>& input)
        {
        ...
        void ResizePlan(const size_t newSize)
        {
            ...
            m_plan = fftw_plan_dft_r2c_1d(m_fftLength, reinterpret_cast<double*>(&m_complexSignal[0]), reinterpret_cast<fftw_complex*>(&m_spectr[0]), FFTW_ESTIMATE);
        }
    ...
    public:
        ...
        ReverseFFTBuilder(const ReverseFFTBuilder& rhs)
            :
              m_fftLength(rhs.m_fftLength),
              m_complexSignal(rhs.m_fftLength),
              m_spectr(rhs.m_fftLength),
              m_plan(nullptr)
        {
            InitPlan();
        }

        std::vector<double> BuildReverseFFT(const std::vector<std::complex<double> >& input)
        {
            std::lock_guard<std::mutex> lck(fftMutex);
        ...
        void ResizePlan(const int newSize)
        {
            ...
            m_plan = fftw_plan_dft_c2r_1d(m_fftLength, (fftw_complex *)&m_spectr[0], (double *)&m_complexSignal[0], FFTW_ESTIMATE);
        }
    ...
    public:
        ...
        virtual void FiltrationProcess(const std::vector<double> &inputSignal, std::vector<double> &outputSignal);

        virtual void SimpleFiltrationProcess(const std::vector<double> &inputSignal, std::vector<double> &outputSignal);

        virtual FilterType GetFilterType() = 0;

        virtual void ChangeParams(size_t order = 0, double freq_cut1 = 0, double freq_cut2 = 0) = 0;
        void ChangeFilter();
    ...
    {
    public:
        BandPassFilter(const size_t filterOrder,const double lowFreqCut, const double hightFreqCut);
        ~BandPassFilter() override = default;

        FilterType GetFilterType() override;

        void ChangeParams(size_t order, double freq_cut1, double freq_cut2) override;
    protected:
        void FilterInit() override;
    private:
    ...
    {
    public:
        BandStopFilter(const size_t filterOrder,const double lowFreqCut, const double hightFreqCut);
    ...
    public:
        ...
        void FiltrationProcess(const std::vector<double> &inputSignal, std::vector<double> &outputSignal) override;

        void SimpleFiltrationProcess(const std::vector<double> &inputSignal, std::vector<double> &outputSignal) override;
...
public:
    ...
    void SetFilterParameters(double SampleFreq, double CutFreq1, double CutFreq2, int order, FilterType type);
