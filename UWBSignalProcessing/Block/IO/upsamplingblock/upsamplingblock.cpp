...
UpsamplingBlock::UpsamplingBlock():
    IOBlock("Upsampling")
{
    setToolTip("Выполняет повышение частоты дискретизации в N раз");
}

AbstractBlock *UpsamplingBlock::Clone() const
{
    return new UpsamplingBlock();
}

AbstractModule* UpsamplingBlock::GetModule() const
{
    return m_module.get();
}

QString UpsamplingBlock::GetType() const
{
...
void UpsamplingBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void UpsamplingBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
