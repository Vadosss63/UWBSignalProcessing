...
AccumulationDialogBoxDual::AccumulationDialogBoxDual(
    AccumulationDualModule *parent, QWidget *wparent)
  ...
  m_widget2 =
      new CustomDialWidget("Число первых и последних накоплений сигнала (не "
                           "задавать больше чем (число накоплений сигнала)/2)");
...
void AccumulationDialogBoxDual::SendCommandAbstractModule() {
  m_module->SetSizeBuf(m_widget->getDisplayIntVal(),
                       m_widget2->getDisplayIntVal());
}
...
QList<std::pair<QString, QVariant>> AccumulationDialogBoxDual::GetSetting() {
  ...
  listSetting.append(
      std::make_pair("m_widget2", m_widget2->getDisplayIntVal()));
  return listSetting;
}

bool AccumulationDialogBoxDual::SetSetting(
    QList<std::pair<QString, QVariant>> listSetting) {
