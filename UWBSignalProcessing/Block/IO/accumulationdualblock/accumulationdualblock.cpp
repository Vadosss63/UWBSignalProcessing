...
{
    setToolTip("Производит сумирование сигнала первые n раз, \n и вычитание последних n раз \n после чего происходит выталкивание \n среднего значения");
}
...
void AccumulationDualBlock::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
...
void AccumulationDualBlock::InitBlock(QWidget *wparent)
{
    ...
    m_dialog = std::make_unique<AccumulationDialogBoxDual>(m_module.get(), wparent);
    CreateBlockPorts();
...
void AccumulationDualBlock::RegistrOperationManager(AbstractOperationManager* operationManager)
{
    if(m_module)
